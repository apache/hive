        ...
        for (Path partPath : partPaths) {
          try {
            if (shouldEnableCm) {
              ...
              //Check if partition directory is empty, delete it
              ...
              //If no cm delete the complete partition directory
              wh.deleteDir(partPath, true, ifPurge, shouldEnableCm);
            }
          } catch (Exception e) {
    ...
    */
        ...
        try {
          ...
          if (!success && madeDir) {
            wh.deleteDir(new Path(part.getSd().getLocation()), true, false,
                    ReplChangeManager.shouldEnableCm(ms.getDatabase(part.getCatName(),
                            part.getDbName()), tbl));
          }
        }
    ...
    public List<PartitionSpec> get_part_specs_by_filter(final String dbName, final String tblName,
      ...
      try {
        ...
        if (is_partition_spec_grouping_enabled(table)) {
          partitionSpecs = MetaStoreUtils.getPartitionspecsGroupedByStorageDescriptor(table, partitions);
        }
        else {
          PartitionSpec pSpec = new PartitionSpec();
          pSpec.setPartitionList(new PartitionListComposingSpec(partitions));
