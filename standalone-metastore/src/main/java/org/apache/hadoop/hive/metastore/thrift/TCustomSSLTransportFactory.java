 ...
 * TSocket and TServerSocket. This class is essentially a copy of the
 * Thrift class TSSLTransportFactory but specific for the TCustom*Socket classes
 * created in Hive.
 */
public class TCustomSSLTransportFactory {
   ...
   */
  public static TCustomServerSocket getServerSocket(int port,
      int clientTimeout, InetAddress ifAddress, HiveTSSLTransportParameters params, int bufferSize
      ) throws TTransportException {
    ...
    return createServer(ctx.getServerSocketFactory(), port, clientTimeout,
        params.clientAuth, ifAddress, params, bufferSize);
  }

  private static TCustomServerSocket createServer(
      SSLServerSocketFactory factory, int port, int timeout, boolean clientAuth,
      InetAddress ifAddress, HiveTSSLTransportParameters params,
      int bufferSize) throws TTransportException {
    try {
      ...
      return new TCustomServerSocket(serverSocket, timeout, bufferSize);
    } catch (Exception e) {
   ...
   * @throws TTransportException
   */
  public static TCustomSocket getClientSocket(String host, int port, int timeout, HiveTSSLTransportParameters params,
      int bufferSize) throws TTransportException {
    if (params == null || !(params.isKeyStoreSet || params.isTrustStoreSet)) {
      throw new TTransportException("Either one of the KeyStore or TrustStore must be set for SSLTransportParameters");
    }
    ...
    SSLContext ctx = createSSLContext(params);
    return createClient(ctx.getSocketFactory(), host, port, timeout, bufferSize);
  }

  private static SSLContext createSSLContext(HiveTSSLTransportParameters params) throws TTransportException {
    SSLContext ctx;
    InputStream in = null;
    InputStream is = null;
  ...
  private static TCustomSocket createClient(SSLSocketFactory factory,
      String host, int port, int timeout, int bufferSize) throws TTransportException {
    try {
      SSLSocket socket = (SSLSocket) factory.createSocket(host, port);
      socket.setSoTimeout(timeout);
      return new TCustomSocket(socket, bufferSize);
    } catch (Exception e) {
   ...
   */
  public static class HiveTSSLTransportParameters {
    ...
    public HiveTSSLTransportParameters() {}
     ...
     * @param cipherSuites
     */
    public HiveTSSLTransportParameters(String protocol, String[] cipherSuites) {
     ...
     * @param clientAuth
     */
    public HiveTSSLTransportParameters(String protocol, String[] cipherSuites, boolean clientAuth) {
      if (protocol != null) {
