...
INSERT INTO NEXT_WRITE_ID (NWI_DATABASE, NWI_TABLE, NWI_NEXT)
    SELECT * FROM
        (SELECT db.NAME, tblName.TBL_NAME FROM DBS db,
            (SELECT tbl.DB_ID, tbl.TBL_NAME FROM TBLS tbl,
                (SELECT TBL_ID FROM TABLE_PARAMS WHERE PARAM_KEY='transactional' AND PARAM_VALUE='true') tblParam
            WHERE tbl.TBL_ID=tblParam.TBL_ID) tblName
        where db.DB_ID=tblName.DB_ID) dbTblName,
        (SELECT NTXN_NEXT FROM NEXT_TXN_ID) nextTxn;
...
INSERT INTO TXN_TO_WRITE_ID (T2W_DATABASE, T2W_TABLE, T2W_TXNID, T2W_WRITEID)
    SELECT * FROM
        (SELECT db.NAME, tblName.TBL_NAME FROM DBS db,
            ...
            WHERE tbl.TBL_ID=tblParam.TBL_ID) tblName
        where db.DB_ID=tblName.DB_ID) dbTblName,
        (SELECT TXN_ID, TXN_ID as WRITE_ID FROM TXNS) nextTxn;

UPDATE TXN_COMPONENTS SET TC_WRITEID = TC_TXNID;
UPDATE COMPLETED_TXN_COMPONENTS SET CTC_WRITEID = CTC_TXNID;

ALTER TABLE TXN_COMPONENTS CHANGE COLUMN TC_WRITEID TC_WRITEID bigint NOT NULL;
ALTER TABLE COMPLETED_TXN_COMPONENTS CHANGE COLUMN CTC_WRITEID CTC_WRITEID bigint NOT NULL;
