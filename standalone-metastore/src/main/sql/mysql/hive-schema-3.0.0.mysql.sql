...
CREATE TABLE `SCHEMA_VERSION_STATE` (
  `SCHEMA_VERSION_STATE_ID` BIGINT NOT NULL,
  `SCHEMA_VERSION_ID` BIGINT NOT NULL,
  `STATE_ID` BIGINT NOT NULL,
  `SEQUENCE` INT NOT NULL,
  `TIMESTAMP` BIGINT NOT NULL,
  `DETAILS` BLOB,
  PRIMARY KEY (SCHEMA_VERSION_ID, STATE_ID, SEQUENCE),
  UNIQUE KEY (id)
);
CREATE TABLE `SCHEMA_FIELD` (
  `SCHEMA_FIELD_ID` BIGINT primary key,
  `SCHEMA_INSTANCE_ID` BIGINT NOT NULL,
  `TIMESTAMP` BIGINT NOT NULL,
  `NAME` VARCHAR(256) NOT NULL,
  `FIELD_NAME_SPACE` VARCHAR(256),
  `TYPE` VARCHAR(256) NOT NULL,
  FOREIGN KEY (SCHEMA_INSTANCE_ID) REFERENCES SCHMEA_VERSION(SCHEMA_VERSION_ID) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE `SCHEMA_SERDES` (
  `SCHEMA_SERDES_ID` BIGINT PRIMARY KEY,
  `DESCRIPTION` TEXT,
  `NAME` TEXT NOT NULL,
  `FILE_ID` TEXT NOT NULL,
  `SERIALIZER_CLASS_NAME` TEXT NOT NULL,
  `DESERIALIZER_CLASS_NAME` TEXT NOT NULL,
  `TIMESTAMP` BIGINT NOT NULL
);

CREATE TABLE `SCHEMA_SERDES_MAPPING` (
  `SCHEMA_METADATA_ID` BIGINT NOT NULL,
  `SCHEMA_SERDES_ID` BIGINT NOT NULL,
  UNIQUE KEY `UK_IDS` (SCHEMA_METADATA_ID, SCHEMA_SERDES_ID)
);
...
CREATE TABLE `SCHEMA_BRANCH_VERSION_MAPPING` (
  ...
  `SCHEMA_VERSION_INFO_ID` BIGINT NOT NULL,
  UNIQUE KEY `UK_SCHEMA_BRANCH_VERSION` (`SCHEMA_BRANCH_ID`, `SCHEMA_VERSION_INFO_ID`),
