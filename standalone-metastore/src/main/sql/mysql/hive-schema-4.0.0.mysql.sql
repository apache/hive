...
CREATE TABLE `SCHEMA_BRANCH` (
  `SCHEMA_BRANCH_ID` BIGINT PRIMARY KEY,
  `NAME` VARCHAR(256) NOT NULL,
  `SCHEMA_METADATA_NAME` VARCHAR(256) NOT NULL,
  `DESCRIPTION` TEXT,
  `TIMESTAMP` BIGINT DEFAULT NULL,
  FOREIGN KEY (`SCHEMA_METADATA_NAME`) REFERENCES `I_SCHEMA` (`name`) ON DELETE CASCADE ON UPDATE CASCADE,
  UNIQUE KEY `UK_SCHEMA_BRANCH`(`NAME`, `I_SCHEMA`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

CREATE TABLE `SCHEMA_BRANCH_VERSION_MAPPING` (
  `SCHEMA_BRANCH_ID` BIGINT NOT NULL,
  `SCHEMA_VERSION_ID` BIGINT NOT NULL,
   UNIQUE KEY `UK_SCHEMA_BRANCH_VERSION` (`SCHEMA_BRANCH_ID`, `SCHEMA_VERSION_ID`),
   FOREIGN KEY (SCHEMA_BRANCH_ID) REFERENCES `SCHEMA_BRANCH` (SCHEMA_BRANCH_ID) ON DELETE CASCADE ON UPDATE CASCADE,
   FOREIGN KEY (SCHEMA_VERSION_ID) REFERENCES `SCHEMA_VERSION` (SCHEMA_VERSION_ID) ON DELETE CASCADE ON UPDATE CASCADE
 ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
