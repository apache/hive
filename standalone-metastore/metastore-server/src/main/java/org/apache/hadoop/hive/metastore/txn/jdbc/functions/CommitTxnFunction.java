...
public class CommitTxnFunction implements TransactionalFunction<TxnType> {
  ...
  public TxnType execute(MultiDataSourceJdbcResource jdbcResource) throws MetaException, NoSuchTxnException, TxnAbortedException {
    ...
    boolean isHiveReplTxn = rqst.isSetReplPolicy() && TxnType.DEFAULT == rqst.getTxn_type();
    //Find the write details for this transaction.
    //Doing it here before the metadata tables are updated below.
    List<TxnWriteDetails> txnWriteDetails = loadTxnWriteDetails(jdbcResource, isHiveReplTxn, txnid);
    ...
    TxnType txnType = getOpenTxnTypeAndLock(jdbcResource, txnid, isReplayedReplTxn);
    if (txnType == null) {
      return null;
    }
    ...
    updateWSCommitIdAndCleanUpMetadata(jdbcResource, txnid, txnType, commitInfo);
  ...
  private List<TxnWriteDetails> loadTxnWriteDetails(MultiDataSourceJdbcResource jdbcResource,
  ...
  private Long resolveTargetTxnId(MultiDataSourceJdbcResource jdbcResource, long sourceTxnId)
      throws MetaException {
    List<Long> targetTxnIds = jdbcResource.execute(
        new TargetTxnIdListHandler(rqst.getReplPolicy(), List.of(sourceTxnId)));
   ...
   */
  private TxnType getOpenTxnTypeAndLock(MultiDataSourceJdbcResource jdbcResource, long txnid,
      boolean isReplayedReplTxn) throws MetaException, NoSuchTxnException, TxnAbortedException {
    TxnType txnType = jdbcResource.execute(
        new GetOpenTxnTypeAndLockHandler(jdbcResource.getSqlGenerator(), txnid));
    if (txnType != null) {
      return txnType;
    }
  ...
  private CommitInfo prepareWriteSetAndCheckConflicts(MultiDataSourceJdbcResource jdbcResource,
      TransactionContext context, long txnid, TxnType txnType, boolean isReplayedReplTxn)
    ...
    if (isUpdateOrDelete) {
       ...
       * see: {@link #updateWSCommitIdAndCleanUpMetadata(MultiDataSourceJdbcResource, long, TxnType, Long, long)}}.
      ...
      if (!rqst.isExclWriteEnabled()) {
        ...
        if (info != null) {
          handleWriteConflict(jdbcResource, context, undoWriteSetForCurrentTxn, info, txnid, commitId,
              isReplayedReplTxn);
        }
      }
  ...
  private WriteSetInfo checkForWriteConflict(MultiDataSourceJdbcResource jdbcResource, long txnid)
      throws MetaException {
  ...
  private void handleWriteConflict(MultiDataSourceJdbcResource jdbcResource, TransactionContext context,
      Object undoWriteSetForCurrentTxn, WriteSetInfo info, long txnid, Long commitId, boolean isReplayedReplTxn)
      throws MetaException, TxnAbortedException {
    //found a conflict, so let's abort the txn
    String committedTxn = "[" + JavaUtils.txnIdToString(info.txnId()) + "," + info.commitId() + "]";
    ...
    String msg = "Aborting [" + JavaUtils.txnIdToString(txnid) + "," + commitId + "]" + " due to a write conflict on " + resource +
        " committed by " + committedTxn + " " + info.currOpType() + "/" + info.opType();
    //remove WRITE_SET info for current txn since it's about to abort
    context.rollbackToSavepoint(undoWriteSetForCurrentTxn);
    LOG.info(msg);
    //TODO: should make abortTxns() write something into TXNS.TXN_META_INFO about this
    int count = new AbortTxnsFunction(Collections.singletonList(txnid), false, false,
        isReplayedReplTxn, TxnErrorMsg.ABORT_WRITE_CONFLICT).execute(jdbcResource);
    if (count != 1) {
  ...
  private void handleCompletedTxnComponents(MultiDataSourceJdbcResource jdbcResource, long txnid, TxnType txnType,
      char isUpdateDelete, boolean isReplayedReplTxn, long sourceTxnId) throws MetaException {
    if (txnType != TxnType.READ_ONLY && !isReplayedReplTxn && !MetaStoreServerUtils.isCompactionTxn(txnType)) {
      ...
      if (rqst.isSetWriteEventInfos() && !rqst.getWriteEventInfos().isEmpty()) {
        jdbcResource.execute(
            new InsertCompletedTxnComponentsCommand(txnid, isUpdateDelete, rqst.getWriteEventInfos()));
      }
      jdbcResource.execute(
  ...
  private long updateDatabaseProp(MultiDataSourceJdbcResource jdbcResource, String catalog, String database, 
    ...
    DbEntityParam dbEntityParam = jdbcResource.getJdbcTemplate().query(query,
        ...
        rs -> rs.next()
            ? new DbEntityParam(
                rs.getLong("DB_ID"), rs.getString("PARAM_KEY"), rs.getString("PARAM_VALUE")
              )
            : null);
  ...
  private long updateTableProp(MultiDataSourceJdbcResource jdbcResource, String catalog, String db, long dbId,
    ...
    DbEntityParam dbEntityParam = jdbcResource.getJdbcTemplate().query(query,
        ...
        rs -> rs.next() 
            ? new DbEntityParam(
                rs.getLong("TBL_ID"), rs.getString("PARAM_KEY"), rs.getString("PARAM_VALUE")
              )
            : null);
  ...
  private void updatePartitionProp(MultiDataSourceJdbcResource jdbcResource, long tableId,
    ...
    RowMapper<DbEntityParam> mapper = (rs, rowNum) ->
        new DbEntityParam(
            rs.getLong("PART_ID"), rs.getString("PARAM_KEY"), rs.getString("PARAM_VALUE")
        );
  ...
  private void updateWSCommitIdAndCleanUpMetadata(MultiDataSourceJdbcResource jdbcResource,
        long txnid, TxnType txnType, CommitInfo commitInfo) throws MetaException {

    List<String> queryBatch = new ArrayList<>(6);
    // update write_set with real commitId
    if (!commitInfo.isEmpty()) {
      queryBatch.add("UPDATE \"WRITE_SET\" SET \"WS_COMMIT_ID\" = " + commitInfo.commitId() +
          " WHERE \"WS_COMMIT_ID\" = " + commitInfo.tempId() + " AND \"WS_TXNID\" = " + txnid);
    }
    ...
    if (txnType == TxnType.SOFT_DELETE || MetaStoreServerUtils.isCompactionTxn(txnType)) {
      // For soft-delete and compaction transactions, we need to set the commit id
      if (commitInfo.isEmpty() && !ConfVars.useMinHistoryWriteId()) {
        // Take a global txn lock to synchronize the openTxn and commitTxn operations
        new AcquireTxnLockFunction(false).execute(jdbcResource);
      }
      queryBatch.add("""
          UPDATE "COMPACTION_QUEUE" SET "CQ_NEXT_TXN_ID" = %s, "CQ_COMMIT_TIME" = %s
          WHERE "CQ_TXN_ID" = %d"""
          .formatted(
              defaultIfNull(commitInfo.commitId(), "(SELECT MAX(\"TXN_ID\") FROM \"TXNS\")"),
  ...
  private void createCommitNotificationEvent(MultiDataSourceJdbcResource jdbcResource, long txnid, TxnType txnType,
    ...
    if (transactionalListeners != null) {
      notifyCommitOrAbortEvent(
          txnid, EventMessage.EventType.COMMIT_TXN, txnType, jdbcResource.getConnection(),
      ...
      if (compactionInfo != null) {
        MetaStoreListenerNotifier.notifyEventWithDirectSql(
            transactionalListeners, EventMessage.EventType.COMMIT_COMPACTION, new CommitCompactionEvent(txnid, compactionInfo),
