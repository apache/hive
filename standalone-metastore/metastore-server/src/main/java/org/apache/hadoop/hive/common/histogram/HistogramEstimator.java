...
public interface HistogramEstimator {

  Logger LOG = LoggerFactory.getLogger(HistogramEstimator.class.getName());

  void reset();

  byte[] serialize();
  
  HistogramEstimator deserialize(byte[] buf);
  ...
  long computeHistogram(); // TODO: plug Ryan's class here

  int lengthFor(JavaDataModel model);

  boolean canMerge(HistogramEstimator o);
