...
public class DefaultSchemaRegistry implements ISchemaRegistry {
  ...
  private SchemaMetadataFetcher createSchemaMetadataFetcher() {
    ...
    public Long addSchemaMetadata(SchemaMetadata schemaMetadata,
        ...
        try {
          if (!throwErrorIfExists) {
            ISchema lookupSchemaMetadata = metaStoreClient.getISchemaByName(Warehouse.DEFAULT_CATALOG_NAME,
                    Warehouse.DEFAULT_DATABASE_NAME, givenSchemaMetadataInfo.getSchemaMetadata().getName());
    ...
    private SchemaBranch getSchemaBranch(SchemaBranchKey schemaBranchKey) throws SchemaBranchNotFoundException {
      ...
      try {
        Collection <ISchemaBranch> iSchemaBranches = metaStoreClient.getSchemaBranchBySchemaName(Warehouse.DEFAULT_CATALOG_NAME,
                                                Warehouse.DEFAULT_DATABASE_NAME ,schemaBranchKey.getSchemaMetadataName());
        ...
        schemaBranch = new SchemaBranch(iSchemaBranches.iterator().next());
      } catch (TException te) {
