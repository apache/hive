...
/**
 * Utility function that constructs a notification filter to match a given catalog name.
 */
public class CatalogFilter extends BasicFilter {
  ...
  boolean shouldAccept(final NotificationEvent event) {
    if (catalogName == null || event.getCatName() == null || catalogName.equalsIgnoreCase(event.getCatName())) {
      return true;
    }
    return false;
  }
