...
public class RetryingHMSHandler implements InvocationHandler {
  ...
  private RetryingHMSHandler(Configuration origConf, IHMSHandler baseHandler, boolean local) throws MetaException {
    ...
    try {
      ...
      MetaException me = ExceptUtils.wrapMetastoreClientException(new Exception(), "init", e);
    ...
    while (true) {
      try {
        ...
        if (cause instanceof Error) {
          ...
          Error error = null;
          try {
            error = new Error("init fatal error: ", cause);
            LOG.error("Fatal error happened calling method init: ", error);
            isRethrow = false;
          } catch (Throwable t) {
            // Suppress..
          }
