...
public class RetryingHMSHandler implements InvocationHandler {
  ...
  private RetryingHMSHandler(Configuration origConf, IHMSHandler baseHandler, boolean local) throws MetaException {
    ...
    try {
      ...
      // It is too disruptive and not a good idea anyway modify clients to catch a Throwable, so we just rethrow...
      Throwable cause = e.getCause();
      if (cause instanceof Error) {
        throw e;
      }
      MetaException me = ExceptUtils.wrapMetastoreClientException(new Exception(), "initialize", cause);
      LOG.error("HMSHandler fatal error: ", me);
      throw me;
    } catch (NoSuchMethodException e) {
      MetaException me = ExceptUtils.wrapMetastoreClientException(new Exception(), "initialize", e);
      LOG.error("HMSHandler fatal error: ", me);
      throw me;
    }
  }
  ...
  public Result invokeInternal(final Object proxy, final Method method, final Object[] args) throws MetaException {
    ...
    while (true) {
      try {
        ...
        Throwable cause = e.getCause();
        if (cause instanceof Error) {
          // Rethrow UndeclaredThrowableException wrapped OOM and other fatal errors.
          throw e;
        }
        if (cause instanceof javax.jdo.JDOException) {
          ...
          MetaException me = ExceptUtils.wrapMetastoreClientException(new Exception(), method.getName(), cause);
        ...
        if (cause instanceof javax.jdo.JDOException) {
          ...
          if (actualCause instanceof javax.jdo.JDOException ||
            ...
            Deadline.clear();
            MetaException me = ExceptUtils.wrapMetastoreClientException(new Exception(), method.getName(), actualCause);
            LOG.error("Error happened calling method " + method.getName() + ": ", me);
            throw me;
          } else {
            MetaException me = ExceptUtils.wrapMetastoreClientException(new Exception(), method.getName(), cause);
      ...
      MetaException me = ExceptUtils.wrapMetastoreClientException(new Exception(), method.getName(), caughtException);
      ...
      try {
        ...
        MetaException interruptMe = ExceptUtils.wrapMetastoreClientException(new Exception(), method.getName(), e);
