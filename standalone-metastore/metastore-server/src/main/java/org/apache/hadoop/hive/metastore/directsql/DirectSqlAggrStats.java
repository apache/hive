...
public class DirectSqlAggrStats {
  ...
  public List<MetaStoreServerUtils.ColStatsObjWithSourceInfo> getColStatsForAllTablePartitions(
    ...
    try (QueryWrapper query = new QueryWrapper(pm.newQuery("javax.jdo.query.SQL", queryText))) {
      ...
      for (Object[] row : list) {
        ...
        colStatsForDB.add(
            new MetaStoreServerUtils.ColStatsObjWithSourceInfo(colStatObj, catName, dbName, tblName, partName));
        Deadline.checkTimeout();
      }
    }
    return colStatsForDB;
