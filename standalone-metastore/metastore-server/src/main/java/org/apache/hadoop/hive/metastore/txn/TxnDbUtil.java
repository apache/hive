...
public final class TxnDbUtil {
  ...
  private static final EnumMap<DatabaseProduct.ProductId, String> DB_EPOCH_FN =
      new EnumMap<DatabaseProduct.ProductId, String>(DatabaseProduct.ProductId.class) {{
        put(DatabaseProduct.ProductId.DERBY, "{ fn timestampdiff(sql_tsi_frac_second, timestamp('" + new Timestamp(0) +
            "'), current_timestamp) } / 1000000");
        put(DatabaseProduct.ProductId.MYSQL, "round(unix_timestamp(now(3)) * 1000)");
        put(DatabaseProduct.ProductId.POSTGRES, "round(extract(epoch from current_timestamp) * 1000)");
        put(DatabaseProduct.ProductId.ORACLE, "(cast(systimestamp at time zone 'UTC' as date) - date '1970-01-01')*24*60*60*1000 " +
            "+ cast(mod( extract( second from systimestamp ), 1 ) * 1000 as int)");
        put(DatabaseProduct.ProductId.SQLSERVER, "datediff_big(millisecond, '19700101', sysutcdatetime())");
      }};

  private static final EnumMap<DatabaseProduct.ProductId, String> DB_SEED_FN =
      new EnumMap<DatabaseProduct.ProductId, String>(DatabaseProduct.ProductId.class) {{
        put(DatabaseProduct.ProductId.DERBY, "ALTER TABLE \"TXNS\" ALTER \"TXN_ID\" RESTART WITH %s");
        put(DatabaseProduct.ProductId.MYSQL, "ALTER TABLE \"TXNS\" AUTO_INCREMENT = %s");
        put(DatabaseProduct.ProductId.POSTGRES, "ALTER SEQUENCE \"TXNS_TXN_ID_seq\" RESTART WITH %s");
        put(DatabaseProduct.ProductId.ORACLE, "ALTER TABLE \"TXNS\" MODIFY \"TXN_ID\" GENERATED BY DEFAULT AS IDENTITY (START WITH %s )");
        put(DatabaseProduct.ProductId.SQLSERVER, "DBCC CHECKIDENT ('txns', RESEED, %s )");
      }};
  ...
  private static boolean truncateTable(Connection conn, Statement stmt, String name) {
    try {
      ...
      String dbProduct = conn.getMetaData().getDatabaseProductName();
      DatabaseProduct databaseProduct = determineDatabaseProduct(dbProduct, null);
      if (databaseProduct.pid == DatabaseProduct.ProductId.POSTGRES || databaseProduct.pid == DatabaseProduct.ProductId.MYSQL) {
  ...
  static void executeQueriesInBatchNoCount(DatabaseProduct dbProduct, Statement stmt, List<String> queries, int batchSize) throws SQLException {
    if (dbProduct.pid == DatabaseProduct.ProductId.ORACLE) {
      int queryCounter = 0;
      StringBuilder sb = new StringBuilder();
      sb.append("begin ");
