...
import static org.apache.hadoop.hive.metastore.DatabaseProduct.DbType.EXTERNAL;
...
public final class TxnDbUtil {
  ...
  private static final EnumMap<DatabaseProduct.DbType, String> DB_EPOCH_FN =
      new EnumMap<DatabaseProduct.DbType, String>(DatabaseProduct.DbType.class) {{
        ...
        put(EXTERNAL, "{ fn timestampdiff(sql_tsi_frac_second, timestamp('" + new Timestamp(0) +
            "'), current_timestamp) } / 1000000");
  ...
  private static final EnumMap<DatabaseProduct.DbType, String> DB_SEED_FN =
      new EnumMap<DatabaseProduct.DbType, String>(DatabaseProduct.DbType.class) {{
        ...
        put(EXTERNAL, "ALTER TABLE \"TXNS\" ALTER \"TXN_ID\" RESTART WITH %s");
  ...
  private static boolean truncateTable(Connection conn, Statement stmt, String name) {
    try {
      ...
      String dbProduct = conn.getMetaData().getDatabaseProductName();
      DatabaseProduct databaseProduct = determineDatabaseProduct(dbProduct, null);
      if (databaseProduct.isPOSTGRES() || databaseProduct.isMYSQL()) {
