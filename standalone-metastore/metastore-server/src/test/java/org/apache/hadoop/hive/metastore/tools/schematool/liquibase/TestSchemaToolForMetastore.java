...
import org.apache.hadoop.hive.metastore.tools.schematool.task.RootTask;
...
public class TestSchemaToolForMetastore {
  ...
  private final SchemaToolTaskFactory schemaToolTaskFactory = new SchemaToolTaskFactory(
  ...
  private MetastoreValidationTask validator;
  ...
  public void setUp() throws Exception {
    ...
    argsBase = "-dbType " + dbms.getDbType() + " -userName " + dbms.getHiveUser() + " -passWord " + dbms.getHivePassword() +
        " -url " + dbms.getJdbcUrl() + " -driver " + dbms.getJdbcDriver();
    System.setProperty("beeLine.system.exit", "true");
    errStream = System.err;
    outStream = System.out;

    SchemaToolCommandLine commandLine = new SchemaToolCommandLine((argsBase + " -validate").split(" "), null);
    context = new TaskContext(System.getProperty("test.tmp.dir", "target/tmp"), conf, commandLine);
    SchemaToolTask task = new RootTask(new NestedScriptParserFactory()).addChild(new LiquibaseContextTask());
    task.executeChain(context);

    validator = new MetastoreValidationTask(new ScriptScannerFactory());
    conn = context.getConnectionToMetastore(false);
  }
  ...
  public void testValidateSequences() throws Exception {
    ...
    // Test empty database
    boolean isValid = validator.validateSequences(conn, context.getParser());
    ...
    executeStriptAsStream(scripts);
    isValid = validator.validateSequences(conn, context.getParser());
    Assert.assertTrue(isValid);
  ...
  public void testValidateSchemaTables() throws Exception {
    execute("-initSchemaTo 1.2.0");

    boolean isValid = validator.validateSchemaTables(conn, context);
    Assert.assertTrue(isValid);

    // upgrade from 1.2.0 schema and re-validate
    execute("-upgradeSchemaFrom 1.2.0");
    isValid = validator.validateSchemaTables(conn, context);
    Assert.assertTrue(isValid);
    ...
    executeStriptAsStream(deleteScripts);
    isValid = validator.validateSchemaTables(conn, context);
    Assert.assertFalse(isValid);

    executeStriptAsStream(restoreScripts);
    isValid = validator.validateSchemaTables(conn, context);
    Assert.assertTrue(isValid);
    ...
    try {
      ...
      validator.validateSchemaTables(bad, context);
  ...
  public void testValidateNullValues() throws Exception {
    ...
    // Test empty database
    boolean isValid = validator.validateColumnNullValues(conn, context.getParser());
    Assert.assertTrue(isValid);

    // Test valid case
    createTestHiveTableSchemas();
    isValid = validator.validateColumnNullValues(conn, context.getParser());
    Assert.assertTrue(isValid);
  ...
  public void testSchemaInitDryRun() throws Exception {
    execute("-initSchemaTo 1.2.0 -dryRun");
    Assert.assertFalse("Dry run shouldn't create actual metastore", validator.validateSchemaVersions(context));
  }
  ...
  public void testSchemaUpgradeDryRun() throws Exception {
    execute("-initSchemaTo 1.2.0");
    execute("-upgradeSchemaFrom 1.2.0 -dryRun");
    Assert.assertFalse("Dry run shouldn't upgrade metastore schema", validator.validateSchemaVersions(context));
  }
  ...
  public void testSchemaInit() throws Exception {
    execute("-initSchemaTo " + SchemaInfo.getRequiredHmsSchemaVersion());
    Assert.assertTrue(validator.validateSchemaVersions(context));
  }
  ...
  public void testSchemaInitOrUgrade2() throws Exception {
    execute("-initSchemaTo 1.2.0");
    execute("-initOrUpgradeSchema -dryRun");
    Assert.assertFalse("Dry run shouldn't upgrade metastore schema", validator.validateSchemaVersions(context));
  }
  ...
  */
  ...
  public void testValidateSchemaVersions() throws Exception {
    execute("-initSchema");
    // Test an invalid case with incompatible versions
    String minimumRequiredVersion = SchemaInfo.getRequiredHmsSchemaVersion();
    String tableName = dbms.getDbType().equals("postgres") ? "databasechangelog" : "DATABASECHANGELOG";
    String[] scripts = new String[] {
        "update " + tableName + " set labels='1.1.0' where labels = '" + minimumRequiredVersion + "'"
    };
    executeStriptAsStream(scripts);
    boolean isValid = validator.validateSchemaVersions(context);
    Assert.assertFalse(isValid);

    scripts = new String[] {
        "update \" + tableName + \" set labels='4.0.0-alpha-5' where labels = '1.1.0'"
    };
    executeStriptAsStream(scripts);
    isValid = validator.validateSchemaVersions(context);
    Assert.assertFalse(isValid);
  ...
  public void testSchemaUpgrade() throws Exception {
    ...
    Assert.assertFalse("Hive operations shouldn't pass with older version schema", validator.validateSchemaVersions(context));
    ...
    // Verify that driver works fine with the latest schema
    Assert.assertTrue(validator.validateSchemaVersions(context));
  }
  ...
  public void testValidateLocations() throws Exception {
    ...
    //check empty DB
    boolean isValid = validator.validateLocations(conn, null, context.getParser());
    Assert.assertTrue(isValid);
    isValid = validator.validateLocations(conn, new URI[] {defaultRoot, defaultRoot2}, context.getParser());
    Assert.assertTrue(isValid);
    ...
    executeStriptAsStream(scripts);
    isValid = validator.validateLocations(conn, null, context.getParser());
    Assert.assertTrue(isValid);
    isValid = validator.validateLocations(conn, new URI[] {defaultRoot, defaultRoot2}, context.getParser());
    Assert.assertTrue(isValid);
    scripts = new String[] {
  ...
  @Test
  public void testHiveMetastoreDbPropertiesTable() throws HiveMetaException, IOException, ParseException {
    execute("-initSchemaTo 3.0.0");
    validateMetastoreDbPropertiesTable();
  }

  @Test
  public void testMetastoreDbPropertiesAfterUpgrade() throws HiveMetaException, IOException, ParseException {
    execute("-initSchemaTo 1.2.0");
    execute("-upgradeSchema");
    validateMetastoreDbPropertiesTable();
  }
  ...
  private void validateMetastoreDbPropertiesTable() throws HiveMetaException {
    boolean isValid = (boolean) validator.validateSchemaTables(conn, context);
    Assert.assertTrue(isValid);
  ...
  private void execute(String taskArgs) throws HiveMetaException, ParseException {
    SchemaToolCommandLine commandLine = new SchemaToolCommandLine((argsBase + " " + taskArgs).split(" "), null);
    SchemaToolTask task = schemaToolTaskFactory.getTask(commandLine);
    task.executeChain(new TaskContext(System.getProperty("test.tmp.dir", "target/tmp"), conf, commandLine));
  }

  private void executeStriptAsStream(String[] lines) throws IOException {
    for (int i = 0; i < lines.length; i++) {
      lines[i] = quoteIdentifiers(lines[i]).replaceAll("'[Nn]'", booleanFalse());
    }
    try (InputStream inputStream = new ByteArrayInputStream(String.join(System.lineSeparator(), lines).getBytes(
        StandardCharsets.UTF_8))) {
      SchemaToolTestUtil.runScript(context.getConnectionInfo(false), inputStream);
    }
  }
}
