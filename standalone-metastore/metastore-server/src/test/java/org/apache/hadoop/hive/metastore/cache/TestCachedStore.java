...
@Category(MetastoreCheckinTest.class) public class TestCachedStore {
  ...
  @Test public void testPrewarm() throws Exception {
    ...
    List<Partition> db1Ptbl1PartitionsOS =
        objectStore.getPartitions(DEFAULT_CATALOG_NAME, db2.getName(), db1Ptbl1.getTableName(),
            GetPartitionsArgs.getAllPartitions());
    ...
    List<Partition> db2Ptbl1PartitionsOS =
        objectStore.getPartitions(DEFAULT_CATALOG_NAME, db2.getName(), db2Ptbl1.getTableName(),
            GetPartitionsArgs.getAllPartitions());
  ...
  @Test public void testCacheUpdate() throws Exception {
    ...
    Assert.assertTrue(db2Tbls.contains(db2Ptbl1.getTableName()));
    // cs_db1_ptntbl1
    db1Ptns = cachedStore.getPartitions(DEFAULT_CATALOG_NAME, db1.getName(), db1Ptbl1.getTableName(),
        GetPartitionsArgs.getAllPartitions());
    Assert.assertEquals(5, db1Ptns.size());
    // cs_db2_ptntbl1
    db2Ptns = cachedStore.getPartitions(DEFAULT_CATALOG_NAME, db2.getName(), db2Ptbl1.getTableName(),
        GetPartitionsArgs.getAllPartitions());
    Assert.assertEquals(25, db2Ptns.size());
    Deadline.startTimer("");
    db2PtnsOS = objectStore.getPartitions(DEFAULT_CATALOG_NAME, db2.getName(), db2Ptbl1.getTableName(),
        GetPartitionsArgs.getAllPartitions());
    Assert.assertTrue(db2Ptns.containsAll(db2PtnsOS));
    // Clean up
  ...
  public void testAggrStatsRepeatedRead() throws Exception {
    ...
    objectStore.deletePartitionColumnStatistics(DEFAULT_CATALOG_NAME, db.getName(), tbl.getTableName(),
        Arrays.asList(Warehouse.makePartName(tbl.getPartitionKeys(), partVals1)), Arrays.asList(colName),
        CacheUtils.HIVE_ENGINE);
    objectStore.deletePartitionColumnStatistics(DEFAULT_CATALOG_NAME, db.getName(), tbl.getTableName(),
        Arrays.asList(Warehouse.makePartName(tbl.getPartitionKeys(), partVals2)), Arrays.asList(colName),
        CacheUtils.HIVE_ENGINE);
    objectStore.dropPartition(DEFAULT_CATALOG_NAME, db.getName(), tbl.getTableName(),
