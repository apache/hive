<?php

/**
 * Autogenerated by Thrift Compiler (0.24.0)
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated
 */

declare(strict_types=1);

namespace metastore;

use Thrift\Base\TBase;
use Thrift\Type\TType;
use Thrift\Type\TMessageType;
use Thrift\Exception\TException;
use Thrift\Exception\TProtocolException;
use Thrift\Protocol\TProtocol;
use Thrift\Protocol\TBinaryProtocol;
use Thrift\Protocol\TBinaryProtocolAccelerated;
use Thrift\Exception\TApplicationException;

class FireEventRequestData
{
    public static bool $isValidate = false;

    public static array $tspec = [
        1 => [
            'var' => 'insertData',
            'isRequired' => false,
            'type' => TType::STRUCT,
            'class' => '\metastore\InsertEventRequestData',
        ],
        2 => [
            'var' => 'insertDatas',
            'isRequired' => false,
            'type' => TType::LST,
            'etype' => TType::STRUCT,
            'elem' => [
                'type' => TType::STRUCT,
                'class' => '\metastore\InsertEventRequestData',
            ],
        ],
        3 => [
            'var' => 'refreshEvent',
            'isRequired' => false,
            'type' => TType::BOOL,
        ],
    ];

    public ?InsertEventRequestData $insertData = null;
    /**
     * @var \metastore\InsertEventRequestData[]
     */
    public ?array $insertDatas = null;
    public ?bool $refreshEvent = null;

    public function __construct(?array $vals = null)
    {
        if (is_array($vals)) {
            if (isset($vals['insertData'])) {
                $this->insertData = $vals['insertData'];
            }
            if (isset($vals['insertDatas'])) {
                $this->insertDatas = $vals['insertDatas'];
            }
            if (isset($vals['refreshEvent'])) {
                $this->refreshEvent = (bool)$vals['refreshEvent'];
            }
        }
    }

    public function getName(): string
    {
        return 'FireEventRequestData';
    }

    public function read(TProtocol $input): int
    {
        $xfer = 0;
        $fname = null;
        $ftype = 0;
        $fid = 0;
        $input->incrementRecursionDepth();
        try {
            $xfer += $input->readStructBegin($fname);
            while (true) {
                $xfer += $input->readFieldBegin($fname, $ftype, $fid);
                if ($ftype == TType::STOP) {
                    break;
                }
                switch ($fid) {
                    case 1:
                        if ($ftype == TType::STRUCT) {
                            $this->insertData = new \metastore\InsertEventRequestData();
                            $xfer += $this->insertData->read($input);
                        } else {
                            $xfer += $input->skip($ftype);
                        }
                        break;
                    case 2:
                        if ($ftype == TType::LST) {
                            $this->insertDatas = [];
                            $_size941 = 0;
                            $_etype944 = 0;
                            $xfer += $input->readListBegin($_etype944, $_size941);
                            for ($_i945 = 0; $_i945 < $_size941; ++$_i945) {
                                $elem946 = null;
                                $elem946 = new \metastore\InsertEventRequestData();
                                $xfer += $elem946->read($input);
                                $this->insertDatas[] = $elem946;
                            }
                            $xfer += $input->readListEnd();
                        } else {
                            $xfer += $input->skip($ftype);
                        }
                        break;
                    case 3:
                        if ($ftype == TType::BOOL) {
                            $xfer += $input->readBool($this->refreshEvent);
                        } else {
                            $xfer += $input->skip($ftype);
                        }
                        break;
                    default:
                        $xfer += $input->skip($ftype);
                        break;
                }
                $xfer += $input->readFieldEnd();
            }
            $xfer += $input->readStructEnd();
        } finally {
            $input->decrementRecursionDepth();
        }

        return $xfer;
    }

    public function write(TProtocol $output): int
    {
        $xfer = 0;
        $output->incrementRecursionDepth();
        try {
            $xfer += $output->writeStructBegin('FireEventRequestData');
            if ($this->insertData !== null) {
                if (!is_object($this->insertData)) {
                    throw new TProtocolException('Bad type in structure.', TProtocolException::INVALID_DATA);
                }
                $xfer += $output->writeFieldBegin('insertData', TType::STRUCT, 1);
                $xfer += $this->insertData->write($output);
                $xfer += $output->writeFieldEnd();
            }
            if ($this->insertDatas !== null) {
                if (!is_array($this->insertDatas)) {
                    throw new TProtocolException('Bad type in structure.', TProtocolException::INVALID_DATA);
                }
                $xfer += $output->writeFieldBegin('insertDatas', TType::LST, 2);
                $output->writeListBegin(TType::STRUCT, count($this->insertDatas));
                foreach ($this->insertDatas as $iter947) {
                    $xfer += $iter947->write($output);
                }
                $output->writeListEnd();
                $xfer += $output->writeFieldEnd();
            }
            if ($this->refreshEvent !== null) {
                $xfer += $output->writeFieldBegin('refreshEvent', TType::BOOL, 3);
                $xfer += $output->writeBool($this->refreshEvent);
                $xfer += $output->writeFieldEnd();
            }
            $xfer += $output->writeFieldStop();
            $xfer += $output->writeStructEnd();
        } finally {
            $output->decrementRecursionDepth();
        }

        return $xfer;
    }
}
