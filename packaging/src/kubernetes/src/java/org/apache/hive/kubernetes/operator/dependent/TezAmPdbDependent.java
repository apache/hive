...
/**
 * PodDisruptionBudget for Tez Application Masters.
 * Ensures at least one Tez AM remains available in the warm pool during voluntary disruptions.
 */
public class TezAmPdbDependent
    extends CRUDKubernetesDependentResource<PodDisruptionBudget, HiveCluster> {

  public TezAmPdbDependent() {
  ...
  protected PodDisruptionBudget desired(HiveCluster hiveCluster,
    ...
    return new PodDisruptionBudgetBuilder()
        ...
        .withNewSpec()
          ...
          .withNewSelector()
            .withMatchLabels(Labels.selectorForComponent(hiveCluster, "tezam"))
          .endSelector()
        .endSpec()
        .build();
  }
  ...
  public static String resourceName(HiveCluster hiveCluster) {
    return hiveCluster.getMetadata().getName() + "-tezam-pdb";
  }
}
