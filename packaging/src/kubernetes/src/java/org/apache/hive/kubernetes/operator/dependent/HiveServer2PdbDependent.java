...
/**
 * PodDisruptionBudget for HiveServer2.
 * Ensures at least one HS2 pod remains available during voluntary disruptions
 * (scale-down, node drain, rolling updates) to prevent query failures.
 */
public class HiveServer2PdbDependent
    extends CRUDKubernetesDependentResource<PodDisruptionBudget, HiveCluster> {

  public HiveServer2PdbDependent() {
  ...
  protected PodDisruptionBudget desired(HiveCluster hiveCluster,
    ...
    return new PodDisruptionBudgetBuilder()
        ...
        .withNewSpec()
          ...
          .withNewSelector()
            .withMatchLabels(Labels.selectorForComponent(hiveCluster, "hiveserver2"))
          .endSelector()
        .endSpec()
        .build();
  }
  ...
  public static String resourceName(HiveCluster hiveCluster) {
    return hiveCluster.getMetadata().getName() + "-hiveserver2-pdb";
  }
}
