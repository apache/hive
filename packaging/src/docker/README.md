...
```shell
export PROFILE=llap # set to llap; unset it or use core to return to simple hs2+metastore mode
docker-compose --profile "${PROFILE:-core}" -f docker-compose.yml down --rmi local # cleanup previous containers and images
...
docker compose --profile "${PROFILE:-core}" -f docker-compose.yml up -d
```

To view LLAP logs:

```shell
docker compose --profile "${PROFILE:-core}" -f docker-compose.yml logs -f llapdaemon1 llapdaemon2
```

To stop and remove the LLAP stack:

```shell
docker compose --profile "${PROFILE:-core}" -f docker-compose.yml down
```
...
docker compose --profile core up -d
docker compose --profile core exec hiveserver2-standalone /bin/bash
