...
```shell
  ...
  POSTGRES_LOCAL_PATH=`mvn help:evaluate -Dexpression=settings.localRepository -q -DforceStdout`/org/postgresql/postgresql/42.5.1/postgresql-42.5.1.jar docker compose up -d
```
Note: If you don't install maven or have problem in resolving the postgres jar, you can always download this jar yourself,
change the `POSTGRES_LOCAL_PATH` to the path to the downloaded jar.
