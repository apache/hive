...
public class TableUtils {
   ...
   * <li>We add the {@link TableUtils#LOCATION} as the table location
   * <li>We add the {@link TableUtils#NAME} as
  ...
  public static Properties getTableProperties(org.apache.hadoop.hive.metastore.api.Table hmsTable, Configuration conf) {
    ...
    getTableProperties(conf, CatalogUtils.getCatalogName(conf))
  ...
  public static Map<String, String> getTableProperties(Configuration conf, String catalogName) {
    ...
    String namedCatalogTablePrefix = CatalogUtils.CATALOG_CONFIG_PREFIX + catalogName + ".table-default.";

    conf.forEach(config -> {
      if (config.getKey().startsWith(TableUtils.TABLE_DEFAULT_CONFIG_PREFIX)) {
        tableProperties.putIfAbsent(
            config.getKey().substring(TableUtils.TABLE_DEFAULT_CONFIG_PREFIX.length()),
            config.getValue());
