...
public class HiveTableOperations extends BaseMetastoreTableOperations
  ...
  public StagingTableOperations toStagingOps() {
    return new StagingTableOperations(conf, metaClients, fileIO, keyManagementClient, catalogName, database, tableName);
  }
  ...
  public EncryptionManager encryption() {
    ...
    if (tableKeyId != null) {
      if (keyManagementClient == null) {
        throw new RuntimeException("Cant create encryption manager, because key management client is not set");
      }
      ...
      encryptionProperties.put(TableProperties.ENCRYPTION_DEK_LENGTH, String.valueOf(encryptionDekLength));
      encryptionManager =
          EncryptionUtil.createEncryptionManager(encryptedKeysFromMetadata, encryptionProperties, keyManagementClient);
    } else {
...
//    encryptionManager = null;
  ...
  private void encryptionPropsFromMetadata(Map<String, String> tableProperties) {
    ...
    if (tableKeyId != null && encryptionDekLength <= 0) {
      ...
      encryptionDekLength =
          (dekLength == null) ? TableProperties.ENCRYPTION_DEK_LENGTH_DEFAULT : Integer.parseInt(dekLength);
    }
  }
  ...
  private void checkEncryptionProperties(String encryptionKeyIdFromHMS, String dekLengthFromHMS) {
    ...
    String encryptionKeyIdFromMetadata = propertiesFromMetadata.get(TableProperties.ENCRYPTION_TABLE_KEY);
    if (!Objects.equals(encryptionKeyIdFromHMS, encryptionKeyIdFromMetadata)) {
      String errMsg =
          String.format("Metadata file might have been modified. Encryption key id %s differs from HMS value %s",
              encryptionKeyIdFromMetadata, encryptionKeyIdFromHMS);
      throw new RuntimeException(errMsg);
    }

    String dekLengthFromMetadata = propertiesFromMetadata.get(TableProperties.ENCRYPTION_DEK_LENGTH);
    if (!Objects.equals(dekLengthFromHMS, dekLengthFromMetadata)) {
      String errMsg = String.format("Metadata file might have been modified. DEK length %s differs from HMS value %s",
