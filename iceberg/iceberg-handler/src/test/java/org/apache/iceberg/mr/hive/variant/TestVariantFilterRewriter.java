...
public class TestVariantFilterRewriter {
  ...
  public void testStripExtractPredicateBecomesAlwaysTrue() {
    Expression expr = Expressions.equal(Expressions.extract("payload", "$.tier", "string"), "gold");
    Assert.assertSame(Expressions.alwaysTrue(), VariantFilterRewriter.stripVariantExtractPredicates(expr));
  }
  ...
  public void testStripExtractFromAndKeepsOtherConjunct() {
    Expression expr =
        Expressions.and(
            Expressions.equal(Expressions.extract("payload", "$.tier", "string"), "gold"),
  ...
  public void testStripExtractUnderNotBecomesAlwaysTrue() {
    Expression expr =
        Expressions.not(Expressions.equal(Expressions.extract("payload", "$.tier", "string"), "gold"));
    Assert.assertSame(Expressions.alwaysTrue(), VariantFilterRewriter.stripVariantExtractPredicates(expr));
  }

  @Test
  public void testRewritesVariantGetToShreddedColumn() throws Exception {
    ExprNodeGenericFuncDesc predicate = equals(variantGet("payload", "$.tier"), stringConst("gold"));
    ...
    Assert.assertTrue(predicate.getChildren().get(0) instanceof ExprNodeGenericFuncDesc);

    ExprNodeDesc lhs = rewritten.getChildren().get(0);
    ...
    Assert.assertEquals(predicate.getChildren().get(0).getTypeInfo(), col.getTypeInfo());
  }

  @Test
  public void testRewritesNestedObjectPath() throws Exception {
    ExprNodeGenericFuncDesc predicate = equals(variantGet("payload", "$.a.b"), stringConst("gold"));
    ExprNodeGenericFuncDesc rewritten = VariantFilterRewriter.rewriteForShredding(predicate);
  ...
  public void testDoesNotRewriteArrayPath() throws Exception {
    ExprNodeGenericFuncDesc predicate = equals(variantGet("payload", "$[0]"), stringConst("gold"));
    ExprNodeGenericFuncDesc rewritten = VariantFilterRewriter.rewriteForShredding(predicate);
  ...
  public void testDoesNotRewriteWhenPathIsNotConstant() throws Exception {
    ExprNodeColumnDesc payload = new ExprNodeColumnDesc(VARIANT_STRUCT, "payload", "t", false);
    ExprNodeColumnDesc path = new ExprNodeColumnDesc(TypeInfoFactory.stringTypeInfo, "path", "t", false);
  ...
  private static ExprNodeGenericFuncDesc equals(ExprNodeDesc left, ExprNodeDesc right) throws Exception {
    return ExprNodeGenericFuncDesc.newInstance(new GenericUDFOPEqual(), List.of(left, right));
  }
  ...
  private static ExprNodeGenericFuncDesc variantGet(String column, String jsonPath) throws Exception {
    ...
    return ExprNodeGenericFuncDesc.newInstance(new GenericUDFVariantGet(), List.of(payload, path));
  }
