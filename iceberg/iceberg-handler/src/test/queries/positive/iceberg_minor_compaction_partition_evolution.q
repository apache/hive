...
alter table ice_orc COMPACT 'minor' and wait file_size_threshold = '400bytes' pool 'iceberg';

describe formatted ice_orc;
show compactions order by 'partition';

-- Now data files' sizes are below file_size_threshold, therefore compaction is needed
alter table ice_orc COMPACT 'minor' and wait file_size_threshold = '500bytes' pool 'iceberg' where fist_name in ('fn1','fn2','fn5','fn6') order by first_name;;
