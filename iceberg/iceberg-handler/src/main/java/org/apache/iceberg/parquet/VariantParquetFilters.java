...
public final class VariantParquetFilters {
  ...
  private static final class VariantPredicateToParquetVisitor
    ...
    public <T> FilterPredicate predicate(UnboundPredicate<T> predicate) {
      ...
      ResolvedVariantPredicate resolved = resolveTypedVariantPredicate(
    ...
    private static boolean isSupportedOperation(Expression.Operation op) {
      return switch (op) {
    ...
    private static VariantColumnPath extractVariantColumnPath(UnboundPredicate<?> predicate) {
      ...
      int typedValueIdx = shreddedColumnPath.indexOf(VariantPathUtil.SHREDDED_VARIANT_TYPED_VALUE_SEGMENT);
      if (typedValueIdx < 0) {
        return null;
      }
      ...
      String typedValuePathStr =
          shreddedColumnPath.substring(typedValueIdx + VariantPathUtil.SHREDDED_VARIANT_TYPED_VALUE_SEGMENT.length());
  ...
  private static ResolvedVariantPredicate resolveTypedVariantPredicate(
      MessageType schema,
      List<String> variantPath,
      List<String> typedValuePath,
      Expression.Operation op,
      Object literalValue) {
    if (variantPath == null || variantPath.isEmpty() ||
          typedValuePath == null || typedValuePath.isEmpty()) {
      return null;
    }
    ...
    for (int i = 0; i < typedValuePath.size(); i++) {
      ...
      if (last) {
        ...
        columnPathSegments.add(ParquetVariantVisitor.TYPED_VALUE);
        return buildResolvedPredicate(columnPathSegments, leaf, op, literalValue);
      }
  ...
  private static ResolvedVariantPredicate buildResolvedPredicate(
      List<String> typedValueColumnPathSegments,
      PrimitiveType typedValueLeaf,
      Expression.Operation op,
      Object literalValue) {
    String typedValueColumnPath = String.join(".", typedValueColumnPathSegments);
    FilterPredicate predicate =
        PrimitivePredicateFactory.toParquetPredicate(typedValueLeaf, typedValueColumnPath, op, literalValue);
    if (predicate == null) {
      return null;
    }
  ...
  private static final class PrimitivePredicateFactory {
    ...
    private static FilterPredicate toParquetPredicate(
        PrimitiveType primitive, String columnPath, Expression.Operation op, Object value) {
    ...
    private static Float asFloat(Expression.Operation op, Object value) {
      if (!(value instanceof Number)) {
        return null;
      }
      ...
      if (op == Expression.Operation.EQ || op == Expression.Operation.NOT_EQ) {
    ...
    private static <C extends Comparable<C>, COL extends Operators.Column<C> & Operators.SupportsLtGt>
        FilterPredicate compare(Expression.Operation op, COL column, C value) {
      return switch (op) {
