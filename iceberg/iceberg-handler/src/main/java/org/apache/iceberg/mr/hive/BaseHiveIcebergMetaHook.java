...
import org.apache.iceberg.hive.TableUtils;
...
public class BaseHiveIcebergMetaHook implements HiveMetaHook {
  ...
  public void preCreateTable(CreateTableRequest request) {
    ...
    this.tableProperties = TableUtils.getTableProperties(hmsTable, conf);
    ...
    if (!Catalogs.hiveCatalog(conf, tableProperties)) {
      ...
      try {
        ...
        if (CatalogUtils.hadoopCatalog(conf, tableProperties) && hmsTable.getSd() != null &&
  ...
  protected void setCommonHmsTablePropertiesForIceberg(org.apache.hadoop.hive.metastore.api.Table hmsTable) {
    if (CatalogUtils.isHadoopTable(conf, tableProperties)) {
      String location = (hmsTable.getSd() != null) ? hmsTable.getSd().getLocation() : null;
      if (location == null && CatalogUtils.hadoopCatalog(conf, tableProperties)) {
