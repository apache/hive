...
public class HiveIcebergStorageHandler implements HiveStoragePredicateHandler, HiveStorageHandler {
  ...
  private boolean writeColStats(List<ColumnStatistics> colStats, Table tbl) {
    try {
      ...
      try (PuffinWriter puffinWriter = Puffin.write(tbl.io().newOutputFile(statsPath))
        ...
        Map<String, Integer> partIndexMap;

        if (!colStats.get(0).getStatsDesc().isIsTblLevel()) {
          List<String> partitions = IcebergTableUtil.getPartitionNames(tbl, Maps.newHashMap(), true);
          partitions.sort(String.CASE_INSENSITIVE_ORDER);
          partIndexMap = IntStream.range(0, partitions.size()).boxed()
            .collect(Collectors.toMap(partitions::get, Function.identity()));
        } else {
          partIndexMap = null;
        }
        ...
        colStats.forEach(statsObj -> {
          ...
          puffinWriter.add(
            new Blob(
              ...
              ImmutableList.of(partIndexMap != null ?
                  partIndexMap.get(statsObj.getStatsDesc().getPartName()) : 0),
              snapshotId,
  ...
  public AggrStats getAggrColStatsFor(org.apache.hadoop.hive.ql.metadata.Table hmsTable, List<String> colNames,
    ...
    if (snapshot != null) {
      ...
      try {
        ...
        LOG.warn("Unable to retrieve aggregated partition stats: ", e);
        return emptyStats;
      }
    }
    return emptyStats;
  ...
  private List<ColumnStatistics> readColStats(Table table, Path statsPath, Set<String> partNames) {
    ...
    try (PuffinReader reader = Puffin.read(table.io().newInputFile(statsPath.toString())).build()) {
      ...
      if (partNames != null) {
        List<String> partitions = IcebergTableUtil.getPartitionNames(table, Maps.newHashMap(), true);
        partitions.sort(String.CASE_INSENSITIVE_ORDER);

        blobMetadata = IntStream.range(0, partitions.size()).boxed()
            .filter(idx -> partNames.contains(partitions.get(idx)))
            .map(blobMetadata::get)
    ...
    if (previousSnapshotId != null && canProvideColStats(tbl, previousSnapshotId)) {
      ...
      for (ColumnStatistics statsObjNew : statsNew) {
        ColumnStatistics statsObjOld = isTblLevel ?
            statsOld.get(0) : oldStatsMap.get(statsObjNew.getStatsDesc().getPartName());
