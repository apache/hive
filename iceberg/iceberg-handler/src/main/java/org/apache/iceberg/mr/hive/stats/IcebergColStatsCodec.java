...
final class IcebergColStatsCodec {
  ...
  static byte[] encodeEntry(ColumnStatisticsObj statsObj) throws IOException {
    ByteBuffer vector = vectorOf(statsObj.getStatsData());
    if (vector == null) {
    ...
    setVector(asStored.getStatsData(), vector);
  ...
  static ColumnStatisticsObj decodeEntry(byte[] raw, boolean withVectors) {
    ...
    if (!withVectors && vectorOf(statsObj.getStatsData()) != null) {
      ...
      setVector(statsObj.getStatsData(), null);
    }
    return statsObj;
  }
  ...
  static ColumnStatisticsObj withoutVectors(ColumnStatisticsObj statsObj) {
    if (vectorOf(statsObj.getStatsData()) == null) {
      return statsObj;
    }
    ColumnStatisticsObj without = statsObj.deepCopy();
    setVector(without.getStatsData(), null);
    return without;
  }

  /** A boolean holds its own counts and a binary has no vector, so neither carries one. */
  private static ByteBuffer vectorOf(ColumnStatisticsData data) {
  ...
  private static void setVector(ColumnStatisticsData data, ByteBuffer vector) {
    switch (data.getSetField()) {
      case LONG_STATS -> data.getLongStats().setBitVectors(vector);
      case DOUBLE_STATS -> data.getDoubleStats().setBitVectors(vector);
      case STRING_STATS -> data.getStringStats().setBitVectors(vector);
      case DECIMAL_STATS -> data.getDecimalStats().setBitVectors(vector);
      case DATE_STATS -> data.getDateStats().setBitVectors(vector);
      case TIMESTAMP_STATS -> data.getTimestampStats().setBitVectors(vector);
