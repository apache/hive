...
class ReadConf<T> {
  ...
  ReadConf(
    ...
    this.totalValues =
        computeTotalValuesAndShouldSkip(
            typeWithIds, statsFilter, dictFilter, bloomFilter, variantRowGroupMayMatch);
  ...
  private long computeTotalValuesAndShouldSkip(
      MessageType typeWithIds,
    ...
    for (int i = 0; i < shouldSkip.length; i += 1) {
      ...
      boolean shouldRead =
          shouldReadRowGroup(
              i, rowGroup, typeWithIds, statsFilter, dictFilter, bloomFilter, variantRowGroupMayMatch);
      this.shouldSkip[i] = !shouldRead;
      if (shouldRead) {
        computedTotalValues += rowGroup.getRowCount();
      }
    }
    return computedTotalValues;
  }

  private boolean shouldReadRowGroup(
      int rowGroupIndex,
      ...
      ParquetBloomRowGroupFilter bloomFilter,
      boolean[] variantRowGroupMayMatch) {
    if (variantRowGroupMayMatch != null && !variantRowGroupMayMatch[rowGroupIndex]) {
      return false;
    }
    ...
    return statsFilter.shouldRead(typeWithIds, rowGroup) &&
