...
public class HiveIcebergOutputCommitter extends OutputCommitter {
  ...
  public void commitTask(TaskAttemptContext originalContext) throws IOException {
    ...
    Map<String, HiveIcebergDeleteWriter> deleteWriters =
        Optional.ofNullable(HiveIcebergDeleteWriter.getWriters(attemptID))
        .orElseGet(() -> {
          LOG.info("CommitTask found no delete writers for output tables: {}, attemptID: {}", outputs, attemptID);
          return ImmutableMap.of();
        });
    ...
    HiveIcebergDeleteWriter.removeWriters(attemptID);
  }
    ...
    try {
      ...
      Tasks.foreach(outputs)
          ...
          .run(output -> {
            ...
            getFiles(dataFiles, numTasks, fileExecutor, table.location(), jobContext, table.io(), false);
  ...
  private void commitTable(FileIO io, ExecutorService executor, JobContext jobContext, String name, String location,
    ...
    if (HiveIcebergStorageHandler.isDelete(conf)) {
      ...
      getFiles(deleteFiles, numTasks, executor, location, jobContext, io, true);
      ...
      Collection<DataFile> dataFiles = new ConcurrentLinkedQueue<>();
      getFiles(dataFiles, numTasks, executor, location, jobContext, io, true);
   ...
   */
  private static <T> void getFiles(Collection<T> files, int numTasks, ExecutorService executor, String location,
