...
/**
 *
 * This patch effectively reverts the change introduced in Iceberg version 1.9.1
 * (related to #11131, "Core: Optimize MergingSnapshotProducer to use referenced manifests to determine if manifest
 * needs to be rewritten"). The original change caused orphan delete files to remain in manifests,
 * which led to incorrect Iceberg table statistics and inaccurate row counts when tables were
 * queried via Hive. This patch aims to correct the handling of orphan positional delete files during manifest
...
abstract class ManifestFilterManager<F extends ContentFile<F>> {
  ...
  boolean containsDeletes() {
    return !deletePaths.isEmpty() || !deleteFiles.isEmpty() || deleteExpression != Expressions.alwaysFalse() ||
  ...
  private boolean canContainDeletedFiles(ManifestFile manifest) {
    ...
    boolean canContainDropBySeq =
        manifest.content() == ManifestContent.DELETES && manifest.minSequenceNumber() < minSequenceNumber;
            ...
            .forEach(
                entry -> {
                  ...
                  if (markedForDelete || evaluator.rowsMightMatch(file)) {
                    boolean allRowsMatch = markedForDelete || evaluator.rowsMustMatch(file);
                    ValidationException.check(
                        allRowsMatch || isDelete, // ignore delete files where some records may not match
                        // the expression
                        "Cannot delete file where some, but not all, rows match filter %s: %s",
                        this.deleteExpression,
                        file.location());
