...
public class HadoopTableOperations implements TableOperations {
    ...
    try {
      ...
      if (!versionCommitSuccess) {
        throw new CommitFailedException(
              "Can not commit newMetaData because version [%s] has already been committed. " +
                    "tempMetaData=[%s],finalMetaData=[%s].Are there other clients running " +
    ...
    if ((currentMaxVersion - nextVersion) > previousVersionsMax && fs.exists(finalMetadataFile)) {
      ...
      throw new CommitStateUnknownException(
            new RejectedExecutionException(
                  String.format(
                        "Commit rejected by server!The current commit version [%d] is much smaller " +
                              "than the latest version [%d].Are there other clients running in " +
  ...
  boolean commitNewVersion(
    ...
    if (!nextVersionIsLatest(nextVersion, maxVersion)) {
      ...
      throw new CommitFailedException(
            "Cannot commit version [%d] because it is smaller or much larger than the current latest version [%d]." +
