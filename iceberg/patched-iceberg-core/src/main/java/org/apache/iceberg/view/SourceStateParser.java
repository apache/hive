...
public class SourceStateParser {
  ...
  public static void toJson(SourceState sourceState, JsonGenerator generator) throws IOException {
    ...
    for (String level : sourceState.nameSpace().levels()) {
  ...
  public static SourceState fromJson(JsonNode node) {
    ...
    JsonNode namespaceNode = JsonUtil.get(NAMESPACE, node);
    Preconditions.checkArgument(
        namespaceNode.isArray(), "Cannot parse namespace from a non-array: %s", node
    );
    String[] levels = JsonUtil.getStringArray(namespaceNode);
