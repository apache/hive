...
public class TestEncodedOrcFile {
  ...
  /**
   * This test is the same as {@link #testFileSystemIsNotInitializedWithKnownTail}
   * except reader.rows() call at the end. That triggers creation of a RecordReader.
   * RecordReader behavior is different in upstream due to
   * https://issues.apache.org/jira/browse/ORC-416
   * If that ORC is backported, this test would work, too.
   * @throws IOException
   */
  @Test
  @Ignore
  public void testFileSystemIsNotInitializedWhenRowsCalledWithKnownTail() throws IOException {
    JobConf conf = new JobConf();
    Path path = new Path("fmock:///testtable/bucket_0");
    conf.set("hive.orc.splits.include.file.footer", "true");
    conf.set("fs.defaultFS", "fmock:///");
