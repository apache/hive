...
public class TestQueueMetricsState {
  ...
  @Test
  public void testApplySnapshotReducesAgeMs() throws InterruptedException {
    QueueMetricsState state = new QueueMetricsState(null, 5000L);
    long initialAge = state.getAgeMs();

    // Poll up to 200ms to ensure time has passed so the age comparison is meaningful
    long deadlineNanos = System.nanoTime() + TimeUnit.MILLISECONDS.toNanos(200);
    while (state.getAgeMs() <= initialAge && System.nanoTime() < deadlineNanos) {
      Thread.sleep(10);
    }
