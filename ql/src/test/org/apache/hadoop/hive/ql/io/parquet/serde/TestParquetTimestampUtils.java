...
public class TestParquetTimestampUtils {
  ...
  public void testJulianDay() {
    ...
    NanoTime nt = NanoTimeUtils.getNanoTime(ts, Optional.of(TimeZone.getDefault().toZoneId()));
    ...
    NanoTime nt1 = NanoTimeUtils.getNanoTime(ts1, Optional.of(TimeZone.getDefault().toZoneId()));
    ...
    NanoTime nt2 = NanoTimeUtils.getNanoTime(ts2, Optional.of(TimeZone.getDefault().toZoneId()));
    ...
    nt1 = NanoTimeUtils.getNanoTime(ts1, Optional.of(TimeZone.getDefault().toZoneId()));
    ...
    nt2 = NanoTimeUtils.getNanoTime(ts2, Optional.of(TimeZone.getDefault().toZoneId()));
  ...
  public void testNanos() {
    ...
    NanoTime nt = NanoTimeUtils.getNanoTime(ts, Optional.of(GMT));
    ...
    nt = NanoTimeUtils.getNanoTime(ts, Optional.of(GMT));
    ...
    NanoTime n2 = NanoTimeUtils.getNanoTime(ts2, Optional.of(GMT));
    NanoTime n1 = NanoTimeUtils.getNanoTime(ts1, Optional.of(GMT));
  ...
  public void testTimezone() {
    ...
    Timestamp ts = Timestamp.ofEpochMilli(cal.getTimeInMillis(), 1);
    ts = TimestampTZUtil.convertTimestampToZone(ts, GMT, US_PACIFIC, false);
     ...
     */
    NanoTime nt = NanoTimeUtils.getNanoTime(ts, Optional.of(US_PACIFIC));
  ...
  public void testTimezoneless() {
    ...
    NanoTime nt1 = NanoTimeUtils.getNanoTime(ts1, Optional.empty());
    ...
    NanoTime nt2 = NanoTimeUtils.getNanoTime(ts2, Optional.empty());
  ...
  private void verifyTsString(String tsString, boolean local) {
    ...
    ZoneId tzId = local ? ZoneOffset.UTC : TimeZone.getDefault().toZoneId();
    NanoTime nt = NanoTimeUtils.getNanoTime(ts, Optional.of(tzId));
  ...
  public void testConvertTimestampToZone() {
    ...
    Timestamp ts1 = TimestampTZUtil.convertTimestampToZone(ts, NEW_YORK, US_PACIFIC, false);
    Assert.assertEquals(Timestamp.valueOf("2017-12-31 21:00:00"), ts1);

    Timestamp ts2 = TimestampTZUtil.convertTimestampToZone(ts, US_PACIFIC, NEW_YORK, false);
    Assert.assertEquals(Timestamp.valueOf("2018-01-01 03:00:00"), ts2);
  }
