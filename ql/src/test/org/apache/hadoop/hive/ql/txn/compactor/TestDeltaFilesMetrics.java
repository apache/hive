...
public class TestDeltaFilesMetrics extends CompactorTest  {
  ...
  private void setUpHiveConf() {
    HiveConf.setIntVar(conf, HiveConf.ConfVars.HIVE_TXN_ACID_METRICS_OBSOLETE_DELTA_NUM_THRESHOLD, 1);
    HiveConf.setIntVar(conf, HiveConf.ConfVars.HIVE_TXN_ACID_METRICS_DELTA_NUM_THRESHOLD, 1);
    HiveConf.setTimeVar(conf, HiveConf.ConfVars.HIVE_TXN_ACID_METRICS_REPORTING_INTERVAL, 1, TimeUnit.SECONDS);
    HiveConf.setFloatVar(conf, HiveConf.ConfVars.HIVE_TXN_ACID_METRICS_DELTA_PCT_THRESHOLD, 0.15f);
    conf.setBoolean(MetastoreConf.ConfVars.METRICS_ENABLED.getVarname(), true);
    conf.setBoolean(MetastoreConf.ConfVars.METASTORE_ACIDMETRICS_EXT_ON.getVarname(), true);
    conf.setBoolean(MetastoreConf.ConfVars.COMPACTOR_INITIATOR_ON.getVarname(), true);
  }
  ...
  public void testDeltaFileMetricPartitionedTable() throws Exception {
    ...
    // 2 active deltas
    // 1 small delta, but it's below threshold count
    // 0 obsolete deltas
    ...
    // 3 active deltas
    // 2 small deltas
    // 3 obsolete deltas (including initial base directory)
    verifyMetricsMatch(new HashMap<String, String>() {{
  ...
  @Test(expected = javax.management.InstanceNotFoundException.class)
  public void testDeltaFilesMetricFromWorkerWithInitiatorOff() throws Exception {
    setUpHiveConf();
    conf.setBoolean(MetastoreConf.ConfVars.COMPACTOR_INITIATOR_ON.getVarname(), false);
    startWorker();
    Assert.assertEquals(0, gaugeToMap(MetricsConstants.COMPACTION_NUM_SMALL_DELTAS).size());
