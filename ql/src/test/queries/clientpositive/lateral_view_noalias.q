...
EXPLAIN CBO SELECT myTab.* from src LATERAL VIEW explode(map('key1', 100, 'key2', 200)) myTab limit 2;
...
EXPLAIN CBO SELECT explode(map('key1', 100, 'key2', 200)) from src limit 2;
...
explain CBO select * from lv_noalias a join lv_noalias b on a.key=b.key;
