...
LOAD DATA LOCAL INPATH '../../data/files/decimal64table.csv' OVERWRITE INTO TABLE vector_decimal64_case_when;
...
explain vectorization detail select sum(NVL(ss_ext_list_price, 1.0BD)) from vector_decimal64_case_when_tmp;
select sum(NVL(ss_ext_list_price, 1.0BD)) from vector_decimal64_case_when_tmp;
set hive.cbo.enable=true;
explain vectorization detail select sum(NVL(ss_ext_list_price, 1.0)) from vector_decimal64_case_when_tmp;
select sum(NVL(ss_ext_list_price, 1.0)) from vector_decimal64_case_when_tmp;
explain vectorization detail select sum(NVL(ss_ext_list_price, 1.0BD)) from vector_decimal64_case_when_tmp;
select sum(NVL(ss_ext_list_price, 1.0BD)) from vector_decimal64_case_when_tmp;
