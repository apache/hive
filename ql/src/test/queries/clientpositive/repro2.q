...
create database chejianer_mask;
create database chejianer_temp;
CREATE EXTERNAL TABLE `chejianer_mask`.`blacklist`(
`id` bigint COMMENT '',
...
;
CREATE EXTERNAL TABLE `chejianer_mask`.`enquiry`(
`id` string COMMENT '',
`bid` string COMMENT '',
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_mask`.`insurance_direct_supply_enquiry_case`(
`id` string,
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_mask`.`order_`(
`id` string,
`order_no` string,
`type` string,
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_mask`.`order_after_sale`(
`id` string,
`order_no` string,
`type` bigint,
`status` bigint,
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_mask`.`order_after_sale_item`(
`id` string COMMENT '',
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_mask`.`order_logistics_package`(
`id` string COMMENT '',
`order_no` string COMMENT '',
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_mask`.`trading_app_source_record`(
`id` string COMMENT '',
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_mask`.`vendor_company`(
`id` string,
`parent_id` string,
`name` string,
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE EXTERNAL TABLE `chejianer_temp`.`cxj_tableau_order_list_month`(
...
;
CREATE TABLE `chejianer_temp`.`hhq_daily_trade_report_unique_order_detail_total`(
`created_at` string,
`paid_time` string,
`cancel_time` string,
`status_update_at` string,
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE TABLE `chejianer_temp`.`hhq_daily_trade_report_unique_order_part_detail`(
...
'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat'
;
CREATE TABLE `chejianer_temp`.`hhq_enquiry_offer_parts_detail`(
`car_brand_type` string,
`enquiry_type` string,
`um_code` string,
`third_institution` string,
...
create table if not exists chejianer_temp.hhq_enquiry_offer_statics as
select 
case cast
(
    vc.status as string
