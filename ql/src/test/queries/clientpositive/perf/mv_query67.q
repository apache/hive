...
CREATE MATERIALIZED VIEW `my_materialized_view_n101` AS
select i_category, i_class, i_brand, i_product_name, d_year, d_qoy, d_moy, s_store_id, coalesce(ss_sales_price*ss_quantity,0) sumsales
from store_sales, date_dim, store, item
where  ss_sold_date_sk=d_date_sk
    and ss_item_sk=i_item_sk
    and ss_store_sk = s_store_sk
...
DROP MATERIALIZED VIEW `my_materialized_view_n101`;



CREATE MATERIALIZED VIEW `my_materialized_view_n102` AS
select i_category
            ,i_class
            ,i_brand
            ,i_product_name
...
DROP MATERIALIZED VIEW `my_materialized_view_n102`;



CREATE MATERIALIZED VIEW `my_materialized_view_n103` AS
select  *
from (select i_category
            ,i_class
            ,i_brand
