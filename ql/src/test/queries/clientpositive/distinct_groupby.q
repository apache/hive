...
select distinct count(a.value), count(b.value) from src1 a join src1 b on a.key=b.key;
...
select distinct sum(value) over () from src1;

explain select distinct * from (select sum(value) over () from src1) as T;
select distinct * from (select sum(value) over () from src1) as T;
...
select distinct count(a.value), count(b.value) from src1 a join src1 b on a.key=b.key group by a.key;
