...
SELECT ss_sold_date_sk, i_product_name, count(*) FROM jethro_store_sales JOIN jethro_item ON ss_item_sk = i_item_sk WHERE ss_list_price = 9.99 and ss_net_profit < -500 GROUP BY ss_sold_date_sk, i_product_name;
...
--SELECT ss_sold_date_sk, i.i_product_name, count(*) FROM jethro_item i JOIN jethro_store_sales ON ss_item_sk = i_item_sk WHERE jethro_store_sales.ss_net_profit < -500 GROUP BY ss_sold_date_sk, i_product_name ORDER BY ss_sold_date_sk DESC, i_product_name LIMIT 10;
