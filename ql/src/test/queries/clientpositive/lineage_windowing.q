...
-- Example 1: Calculating a running sum of col_002 within each partition
create view b_v_18 as
select *
from (select col_001, sum(src.col_002) over (partition by src.p1 order by src.col_001) as r_num
        from source_tbl2 src) v1;

-- Example 2: Comparing a row's value to the previous row's value
...
-- Example 3: Finding the average of the current and two previous rows (rolling average)
...
-- Example 4: Partition by two columns
