...
CREATE TABLE TEST(
  ...
  col3 decimal CHECK (col3 > 1) enable novalidate rely,
  col4 decimal NOT NULL,
  col5 varchar(100),
  primary key(col1, col2) disable novalidate,
  constraint c3_c4_check CHECK(col3 + col4 > 3))
ROW FORMAT SERDE
...
CREATE TABLE TEST2(
 col varchar(100),
 primary key(col) disable novalidate)
ROW FORMAT SERDE
'org.apache.hadoop.hive.ql.io.orc.OrcSerde'
STORED AS INPUTFORMAT
'org.apache.hadoop.hive.ql.io.orc.OrcInputFormat'
...
CREATE TABLE TEST3(
  ...
  foreign key(col3) references TEST2(col) disable novalidate)
ROW FORMAT SERDE
  'org.apache.hadoop.hive.ql.io.orc.OrcSerde'
STORED AS INPUTFORMAT
  'org.apache.hadoop.hive.ql.io.orc.OrcInputFormat'
