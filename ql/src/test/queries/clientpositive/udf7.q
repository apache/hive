--! qt:dataset:src
CREATE TABLE dest1_n111(c1 STRING) STORED AS TEXTFILE;

FROM src INSERT OVERWRITE TABLE dest1_n111 SELECT '  abc  ' WHERE src.key = 86;
...
SELECT ROUND(LN(3.0),12), LN(0.0), LN(-1), ROUND(LOG(3.0),12), LOG(0.0),
       ...
       POW(CAST (2 AS DECIMAL), CAST(3 AS INT)) FROM dest1_n111;
