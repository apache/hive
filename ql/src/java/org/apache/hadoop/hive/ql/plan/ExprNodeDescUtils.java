...
public class ExprNodeDescUtils {
  ...
  /**
   * Recursively flattens associative AND / OR operators within an ExprNodeDesc tree.
   * For example, it transforms: AND(AND(A, B), C) => AND(A, B, C)
   */
  public static ExprNodeDesc flattenAndOr(ExprNodeDesc expr) {
    if (expr == null || expr.getChildren() == null || expr.getChildren().isEmpty()) {
      return expr;
    }

    // Deeply flatten all children first
    List<ExprNodeDesc> flattenedChildren = new ArrayList<>();
    for (ExprNodeDesc child : expr.getChildren()) {
      flattenedChildren.add(flattenAndOr(child));
    }

    // Handle operator flattening if the current node is a function descriptor
    if (expr instanceof ExprNodeGenericFuncDesc) {
      ExprNodeGenericFuncDesc funcDesc = (ExprNodeGenericFuncDesc) expr;
      GenericUDF udf = funcDesc.getGenericUDF();

      // Handle continuous nested AND chains
      if (udf instanceof GenericUDFOPAnd) {
        List<ExprNodeDesc> collectedAnds = new ArrayList<>();
        gatherOperands(flattenedChildren, GenericUDFOPAnd.class, collectedAnds);
      ...
      // Handle continuous nested OR chains
      if (udf instanceof GenericUDFOPOr) {
        List<ExprNodeDesc> collectedOrs = new ArrayList<>();
        gatherOperands(flattenedChildren, GenericUDFOPOr.class, collectedOrs);
        return new ExprNodeGenericFuncDesc(
            funcDesc.getTypeInfo(),
            funcDesc.getGenericUDF(),
            funcDesc.getFuncText(),
  ...
  /**
   * Helper method to un-nest children matching the specified logical target operator class.
   */
  private static void gatherOperands(List<ExprNodeDesc> nodes, Class<? extends GenericUDF> targetUdf,
      List<ExprNodeDesc> result) {
    for (ExprNodeDesc node : nodes) {
      if (node instanceof ExprNodeGenericFuncDesc) {
        ...
        if (targetUdf.isInstance(funcDesc.getGenericUDF())) {
          // Recursively unwrap children if they match the same logical type (AND/OR)
          gatherOperands(funcDesc.getChildren(), targetUdf, result);
          continue;
        }
      }
      result.add(node);
