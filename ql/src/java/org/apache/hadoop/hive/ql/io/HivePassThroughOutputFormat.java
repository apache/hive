...
public class HivePassThroughOutputFormat<K, V> implements Configurable, HiveOutputFormat<K, V>{
  ...
  public static final String HIVE_PASSTHROUGH_OF_CLASS = "HivePassThroughOutputFormat.class";
  ...
  private void CreateActualOF() throws IOException {
    ...
    try {
      ...
      if (actualOutputFormatClass != null)
       {
        cls =
           (Class<? extends OutputFormat>) Class.forName(actualOutputFormatClass, true, JavaUtils.getClassLoader());
      } else {
    ...
    OutputFormat<? super WritableComparable<?>, ? super Writable> actualOF =  (OutputFormat<? super WritableComparable, ? super Writable>) ReflectionUtils.newInstance(cls, this.getConf());
  ...
  public org.apache.hadoop.mapred.RecordWriter<K, V> getRecordWriter(FileSystem ignored, JobConf job,
    ...
    return (RecordWriter<K, V>) this.actualOutputFormat.getRecordWriter(ignored, job, name, progress);
  }
  ...
  public void setConf(Configuration config) {
    if (config.get("StorageHandlerOF") != null) {
      actualOutputFormatClass = config.get("StorageHandlerOF");
    }
    this.conf = config;
  }
}
