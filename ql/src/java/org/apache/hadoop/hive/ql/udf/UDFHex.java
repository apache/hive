...
public class UDFHex extends UDF { 
  ...
  private Text evaluate(long num) {
    ...
    do {
      len++;
      value[value.length-len] = (byte)Character.toUpperCase(
                                Character.forDigit((int)(num & 0xF), 16));
      num >>>= 4;
    } while(num != 0);
  ...
  public Text evaluate(Text s) {
    ...
    for(int i = 0; i < s.getLength(); i++) {
      value[i*2] = (byte)Character.toUpperCase(
                   Character.forDigit((str[i]&0xF0)>>>4, 16));
      value[i*2 + 1] = (byte)Character.toUpperCase(
                       Character.forDigit(str[i]&0x0F, 16));
    }
