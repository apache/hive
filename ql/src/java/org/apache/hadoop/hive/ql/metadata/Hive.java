...
public class Hive implements AutoCloseable {
  ...
  private static TableFetcher getIcebergTables(IMetaStoreClient msc, String catalogName, String dbPattern,
      String tablePattern) {
   ...
   * @param pattern A pattern to match for the table names.If null, returns all names from this DB.
   * @param type The type of tables to return. VIRTUAL_VIEWS for views. If null, returns all tables and views.
   * @param ifIceberg If to only return iceberg tables.
   * @return list of table names that match the pattern.
   * @throws HiveException
   */
  public List<String> getTablesByType(String dbName, String pattern, TableType type, boolean ifIceberg)
      throws HiveException {
    ...
    try {
      ...
      if (ifIceberg) {
        if (type != null) {
          ...
          if (pattern != null) {
            return Lists.transform(getIcebergTables(getMSC(), null, dbName, pattern).getTables(),
                    TableName::getTable);
          } else {
            return Lists.transform(getIcebergTables(getMSC(), null, dbName, ".*").getTables(),
                    TableName::getTable);
          }
        }
      }
