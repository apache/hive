 ...
 */
public class VectorMapJoinFastStringHashSetParallel extends VectorMapJoinFastHashTableWrapper implements
    VectorMapJoinBytesHashSet {

  private VectorMapJoinFastStringHashSet[] vectorMapJoinFastStringHashSets;
  ...
  public void putRow(BytesWritable currentKey, BytesWritable currentValue, long hashCode, long key)
      throws HiveException, IOException {
    vectorMapJoinFastStringHashSets[(int) (((1 << 2) - 1) & (hashCode >> 62 - 1))].putRow(currentKey, currentValue,
        hashCode, key);
  }
  ...
  public VectorMapJoinFastStringHashSetParallel(
      boolean isFullOuter,
      int initialCapacity, float loadFactor, int writeBuffersSize, long estimatedKeyCount, TableDesc tableDesc) {
    vectorMapJoinFastStringHashSets = new VectorMapJoinFastStringHashSet[4];
    for (int i=0; i<4; ++i) {
  ...
  @Override public JoinUtil.JoinResult contains(byte[] keyBytes, int keyStart, int keyLength,
    ...
    return vectorMapJoinFastStringHashSets[(int) (((1 << 2) - 1) & (hashCode >> 62 - 1))].contains(keyBytes, keyStart, keyLength, hashSetResult);
  }
