...
public class HiveMetaStoreAuthorizer extends MetaStorePreEventListener {
  ...
  void checkPrivileges(final HiveMetaStoreAuthzInfo authzContext, HiveAuthorizer authorizer) throws MetaException {
    if (LOG.isDebugEnabled()) {
      LOG.debug("==> HiveMetaStoreAuthorizer.checkPrivileges(): authzContext=" + authzContext + ", authorizer=" + authorizer);
    }

    String user = authzContext.getUGI().getShortUserName();
    validateOperationForUser(preEventContext, user);
  ...
  private boolean isSuperUser(String userName) {
    ...
    if (MetaStoreServerUtils.checkUserHasHostProxyPrivileges(userName, conf, ipAddress)) {
      ret = true;
    }
  ...
  private void validateOperationForUser(PreEventContext preEventContext, String user) throws  MetaException {
    if (preEventContext != null) {
      ...
      switch (preEventType) {
        ...
        case ALTER_CATALOG:
        case DROP_CATALOG:
          throw new MetaException("Operation type " + preEventType.name() + "not allowed for user:" + user);
        case CREATE_TABLE:
          if (isViewOperation(preEventContext)) {
            throw new MetaException("CREATE VIEW not allowed for user:" + user);
          }
        case ALTER_TABLE:
          if (isViewOperation(preEventContext)) {
            throw new MetaException("ALTER VIEW not allowed for user:" + user);
          }
        case DROP_TABLE:
          if (isViewOperation(preEventContext)) {
            throw new MetaException("DROP VIEW not allowed for user:" + user);
          }
        default:
          break;
      }
  ...
  private boolean isViewOperation(PreEventContext preEventContext) {
    boolean ret = false;
