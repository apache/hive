...
public class GenericUDFArrayJoin extends AbstractGenericUDFArrayBase {
  ...
  @Override public Object evaluate(DeferredObject[] arguments) throws HiveException {
    ...
    if (arguments[REPLACE_NULL_IDX].get() != null) {
      return converter.convert(
          Joiner.on(separator).useForNull(arguments[REPLACE_NULL_IDX].get().toString()).join(retArray));
    } else {
      return converter.convert(Joiner.on(separator).skipNulls().join(retArray));
    }
  }
}
