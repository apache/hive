...
@Description(name = "bitmap_count", value = "_FUNC_(x) - Returns the cardinality of input bitmaps.\n" ,
extended = "Example:\n > select _FUNC_(bitmap_from_string('1,2,3'));\n 3"
)
public class UDFBitmapCount extends GenericUDF {
  public final static String FUNC_NAME = "bitmap_count";
  private static final Logger LOG = LoggerFactory.getLogger(UDFBitmapCount.class.getName());
  ...
  @Override public Object evaluate(DeferredObject[] arguments) throws HiveException {
    try {
      return new LongWritable(RoaringBitmapSerDe.deserialize(GenericUDFParamUtils.getBinaryValue(arguments, 0, converters).getBytes()).getLongCardinality());
    } catch (IOException e) {
