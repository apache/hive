...
public class ShowLocksAnalyzer extends BaseSemanticAnalyzer {
  ...
  public void analyzeInternal(ASTNode root) throws SemanticException {
    ...
    if (root.getChildCount() >= 1) {
      ...
      for (int i = 0; i < root.getChildCount(); i++) {
        ...
        if (child.getType() == HiveParser.TOK_TABTYPE) {
          qualifiedTableName = DDLUtils.getFQName((ASTNode) child.getChild(0));
          // get partition metadata if partition specified
          if (child.getChildCount() == 2) {
            ASTNode partitionSpecNode = (ASTNode) child.getChild(1);
            partitionSpec = getValidatedPartSpec(getTable(qualifiedTableName), partitionSpecNode, conf, false);
          }
    ...
    if (qualifiedTableName != null) {
      ...
      TableName table = TableName.fromString(qualifiedTableName, HiveUtils.getCurrentCatalogOrDefault(conf),
          defaultDatabase);
      catName = table.getCat();
      dbName = table.getDb();
