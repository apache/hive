...
public class ProtobufMessageExtractor implements Extractor {

  private Configuration conf;
  private final BodyConverter bodyConverter;

  public ProtobufMessageExtractor(final ConstCode formatCode) {
    if (formatCode == ConstCode.p) {
      bodyConverter = new ProtobufBodyConverter();
    } else {
  ...
  public void extractIdentifyFieldValues(final Writable fldName, final WritableComparable msgId, final Writable body, final Set<WritableComparable> set) {
    ...
    GeneratedMessageV3 msg = (GeneratedMessageV3) bodyConverter.convertBody(msgId, body);
    ProtobufMessageExtractor.extract2(fieldName, msg, set);
  }

  @Override
  public void extract(Writable fldName, Object msg, Set<WritableComparable> ids) {
    String fieldName = fldName.toString();
    ProtobufMessageExtractor.extract2(fieldName, (GeneratedMessageV3) msg, ids);
  }
  ...
  public static void extract2(final String idFieldName, final GeneratedMessageV3 msg, final Set<WritableComparable> set) {
    final String normalizedFieldName = idFieldName + "_";
    ...
    for (Field field : fields) {
      ...
      if (field.getName().equals(normalizedFieldName)) {
        ...
        if (GeneratedMessageV3.class.isAssignableFrom(fieldType)) {
          try {
            field.setAccessible(true);
            final GeneratedMessageV3 sub = (GeneratedMessageV3) field.get(msg);
            if (sub != null) {
          ...
          if (GeneratedMessageV3.class.isAssignableFrom(ptc)) {
            try {
              field.setAccessible(true);
              ...
              if (lst != null) {
                for (Object o : lst) {
                  if (o != null) {
                    extract2(idFieldName, (GeneratedMessageV3) o, set);
                  }
                }
              }
            } catch (IllegalAccessException e) {
  ...
  private static void getFieldValue2(Field f, GeneratedMessageV3 msg, Set<WritableComparable> set) {
    if (f == null) {
      return;
    }
  ...
  public static boolean contains2(final Class<? extends GeneratedMessageV3> clazz, final String fieldName) {
    final String normalizedFieldName = fieldName + "_";
    Field[] fields = clazz.getDeclaredFields();
    for (Field field : fields) {
      ...
      if (GeneratedMessageV3.class.isAssignableFrom(type)) {
        if (contains2((Class<? extends GeneratedMessageV3>) type, fieldName)) {
          return true;
        }
      }
      if (List.class.isAssignableFrom(type)) {
        ...
        if (GeneratedMessageV3.class.isAssignableFrom(cat)) {
          if (contains2((Class<? extends GeneratedMessageV3>) cat, fieldName)) {
            return true;
          }
        }
      }
