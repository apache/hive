...
@Description(name = "to_bitmap", value = "_FUNC_(x) - Compute intersection of two or more input bitmaps and  return the size of the result size.",extended = "Example:\n"
    + "  > SELECT _FUNC_(bitmap_from_string('1,2,3'),bitmap_from_string('2,3,4'));\n"
    + "  2\n"
    + "  > SELECT _FUNC_(bitmap_from_string('1,2,3'),bitmap_from_string('2,3,4'),bitmap_from_string('3,4,5'));\n"
    + "  2\n"
    + "  > SELECT _FUNC_(bitmap_from_string('1,2,3'),bitmap_from_string('4,5,6'));\n"
    + "  0\n"
    + "  > SELECT _FUNC_(bitmap_from_string('1,2,3'),bitmap_from_string('1,2,3'));\n"
    + "  3\n"
    + "  > SELECT _FUNC_(bitmap_from_string('1,2,3'),bitmap_from_string('1,2,3'),bitmap_from_string('1,2,3'));\n"
    + "  3\n"
    + "  > SELECT _FUNC_(bitmap_from_string('1,2,3'),bitmap_from_string('1,2,3'),bitmap('1,2,3'),bitmap_from_string('1,2,3'));\n"
    + "  3\n")
public class UDFBitmapAndCount extends GenericUDF {
  public final static String FUNC_NAME = "bitmap_and_count";
  private static final Logger LOG = LoggerFactory.getLogger(UDFBitmapAndCount.class.getName());
  ...
  @Override public Object evaluate(DeferredObject[] arguments) throws HiveException {
    try {
      ...
      for (int i = 1; i < arguments.length; i++) {
        result.and(RoaringBitmapSerDe.deserialize(GenericUDFParamUtils.getBinaryValue(arguments, i, converters).getBytes()));}
