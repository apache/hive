...
public class PartitionPruner extends Transform {
  ...
  private static boolean isPartitionKey(Table table, String columnName) {
    List<String> partitionKeyNames = table.getStorageHandler() != null && table.getStorageHandler().alwaysUnpartitioned() ?
        ...
        .map(fieldSchema -> fieldSchema.getName()).distinct().collect(Collectors.toList()) : table.getPartCols().stream().map(FieldSchema::getName).collect(
        Collectors.toList());
    return partitionKeyNames.stream().filter(item->item.equalsIgnoreCase(columnName)).findFirst().isPresent();
  }
  /**
