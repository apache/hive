...
public final class IoTrace {
  ...
  public static final int TREE_READER_NEXT_VECTOR = 1, READING_STRIPE = 2,
      INDEX_UPDATE = 3, SARG_RESULT = 4, RANGES = 5, COLUMN_READ = 6, SKIP_STREAM = 7,
  ...
  private static int dumpOneLine(int ix, Logger logger, long[] log) {
    ...
    // Note: we don't actually log anything useful here since it will soon be removed.
    case INDEX_UPDATE: {
      logger.info(ix + ": Updating stale indexes for stripe " + getSecondInt(log[ix]));
      return ix + 1;
    }
    case SARG_RESULT: {
  ...
  // Note: we don't actually log anything useful here since it will soon be removed.
  public void logIndexUpdate(int stripeIx, boolean[] globalIncludes) {
    if (log == null) return;
    int offset = this.offset;
    if (offset + 1 > log.length) return;
    log[offset] = makeIntPair(INDEX_UPDATE, stripeIx);
    this.offset += 1;
  }
