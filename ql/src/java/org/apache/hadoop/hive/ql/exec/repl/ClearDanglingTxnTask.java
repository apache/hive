...
public class ClearDanglingTxnTask extends Task<ClearDanglingTxnWork> implements Serializable {
    ...
    private Logger LOG = LoggerFactory.getLogger(ClearDanglingTxnTask.class);
    ...
    public int execute() {
        ...
        Map<String, String> replTxnEntries = null;
        try {
            replTxnEntries = txnManager.getReplayedTxnsForPolicy(replPolicy);
        } catch (LockException e) {
            throw new RuntimeException("Failed to get entries from repl_txn_map: " + e);
        }
        ...
        if (conf.getBoolVar(HiveConf.ConfVars.HIVE_REPL_CLEAR_DANGLING_TXNS_ON_TARGET)) {
            try {
                ...
                throw new RuntimeException("Failed to clear dangling transactions from repl_txn_map: " + e);
            }
        } else {
    ...
    private void clearDanglingTxns(Map<String, String> replTxnEntries, HiveTxnManager txnManager, String replPolicy) throws IOException, LockException {
        ...
        if (!fs.exists(path)) {
            LOG.warn("Path " + path + " does not exist. Skipping clearing dangling transactions from repl_txn_map.");
            return;
        }
