...
public class TabularOutputFormat implements HiveOutputFormat<KeyWritable, ValueWritable>
{
  private static final Logger LOG = LoggerFactory.getLogger(TabularOutputFormat.class);
  private TabularIndex tabularIndex;
  ...
  public FileSinkOperator.RecordWriter getHiveRecordWriter(JobConf jc, Path finalOutPath, Class<? extends Writable> valueClass,
    ...
    tabularIndex = new TabularIndex(jc);
    return new BtreeRecWriter(finalOutPath, jc, tabularIndex);
  }

  private static class BtreeRecWriter implements FileSinkOperator.RecordWriter {

    private final OutputStream os;
    private final TabularIndex index;

    BtreeRecWriter(Path path, JobConf jc, TabularIndex index) throws IOException {
    ...
    public void write(final Writable w) throws IOException {
      ...
      StructValueList value = struct.getValue();

      TabularEntry dataEntry = new TabularEntry(key, value);
