...
public class LateralViewPlan {
  ...
  public LateralViewPlan(ASTNode lateralView, RelOptCluster cluster, RelNode inputRel,
    ...
    RexCall udtfCall = getUDTFFunction(functionAST, inputRR, inputRel);
  ...
  private RexCall getUDTFFunction(ASTNode functionAST,
      RowResolver inputRR, RelNode inputRel) throws SemanticException {
  ...
  private String getTableAliasFromASTNode(ASTNode selExprClause) throws SemanticException {
    ...
    for (Object obj : selExprClause.getChildren()) {
  ...
  private RelDataType getRetType(RelOptCluster cluster, RelNode inputRel,
      RexNode udtfCall, List<String> columnAliases) throws SemanticException {
    ...
    for (RelDataTypeField field : inputRel.getRowType().getFieldList()) {
      allDataTypes.add(field.getType());
      allDataTypeNames.add(field.getName());
    }
    ...
    // return type.
    int i = 0;
    for (RelDataTypeField relDataTypeField : retType.getFieldList()) {
      allDataTypes.add(relDataTypeField.getType());
      allDataTypeNames.add(columnAliases.get(i++));
    }
