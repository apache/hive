...
public class RowLineageUtils {
  ...
  public static void addRowLineageColumnsForUpdate(Table table, MultiInsertSqlGenerator sqlGenerator,
    ...
    if (supportsRowLineage(table)) {
      setRowLineageColumns(true, sqlGenerator, conf);
  ...
  public static void addSourceColumnsForRowLineage(boolean isRowLineageSupported, MultiInsertSqlGenerator sqlGenerator,
    ...
    if (isRowLineageSupported) {
      ...
      sqlGenerator.append(", ");
      sqlGenerator.append(
          HiveUtils.unparseIdentifier(prefix + VirtualColumn.LAST_UPDATED_SEQUENCE_NUMBER.getName(), conf));
    }
  }
  ...
  public static void addSourceForRowLineageCopyOnWriteMerge(boolean isRowLineageSupported,
      MultiInsertSqlGenerator sqlGenerator, Configuration conf) {
    if (isRowLineageSupported) {
      sqlGenerator.append(", ");
      sqlGenerator.append(HiveUtils.unparseIdentifier(VirtualColumn.ROW_LINEAGE_ID.getName(), conf)).append(" AS ")
          .append(HiveUtils.unparseIdentifier(TARGET_PREFIX + VirtualColumn.ROW_LINEAGE_ID.getName(), conf));
      sqlGenerator.append(", ");
      sqlGenerator.append(HiveUtils.unparseIdentifier(VirtualColumn.LAST_UPDATED_SEQUENCE_NUMBER.getName(), conf))
          .append(" AS ").append(
    ...
    public static void addValuesForRowLineageForCopyOnMerge(boolean isRowLineageSupported, List<String> values,
        String lastUpdatedSequenceNumber, Configuration conf) {
      if (isRowLineageSupported) {
        values.add(HiveUtils.unparseIdentifier(TARGET_PREFIX + VirtualColumn.ROW_LINEAGE_ID.getName(), conf));
        values.add(lastUpdatedSequenceNumber);
      }
    }
}