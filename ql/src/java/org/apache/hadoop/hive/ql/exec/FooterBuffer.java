...
public class FooterBuffer {
  ...
  public boolean updateBuffer(JobConf job, RecordReader recordreader,
    ...
    key = ReflectionUtils.copy(job, (WritableComparable)buffer.get(cur).getKey(), key);
    value = ReflectionUtils.copy(job, (Writable)buffer.get(cur).getValue(), value);
    boolean notEOF = recordreader.next(buffer.get(cur).getLeft(), buffer.get(cur).getValue());
    if (notEOF) {
      cur = (++cur) % buffer.size();
    }
    return notEOF;
