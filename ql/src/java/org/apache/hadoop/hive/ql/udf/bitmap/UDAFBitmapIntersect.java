...
@Description(name = "bitmap_intersect", value = "_FUNC_(x) - the intersection of the set of bitmap values and return bitmap."
        , extended = " Example:\n > select city,_FUNC_(id) from src group by city;\n"
)
public class UDAFBitmapIntersect extends AbstractGenericUDAFResolver implements Bitmap {
  public final static String FUNC_NAME = "bitmap_intersect";
  private static final Logger LOG = LoggerFactory.getLogger(UDAFBitmapIntersect.class.getName());
...
//      GenericUDFParamUtils.obtainBinaryConverter(parameters, 0, inputTypes, converters);
    ...
    @Override public void iterate(AggregationBuffer agg, Object[] parameters) throws HiveException {
      try {
          BitmapDataAggregationBuffer bitmapBuffer = (BitmapDataAggregationBuffer) agg;
          if (bitmapBuffer.isFirstAccess()) {
              bitmapBuffer.bitmap.or(RoaringBitmapSerDe.deserialize(midOI.getPrimitiveJavaObject(parameters[0])));
          }else{
              bitmapBuffer.bitmap.and(RoaringBitmapSerDe.deserialize(midOI.getPrimitiveJavaObject(parameters[0])));
          }
      } catch (IOException e) {
    ...
    @Override public Object terminatePartial(AggregationBuffer agg) throws HiveException {
      return new BytesWritable(RoaringBitmapSerDe.serialize(((Bitmap.BitmapDataAggregationBuffer) agg).bitmap).array());
    }

    @Override public void merge(AggregationBuffer agg, Object partial) throws HiveException {
      try {
        ((BitmapDataAggregationBuffer) agg).bitmap.and(RoaringBitmapSerDe.deserialize(midOI.getPrimitiveJavaObject(partial)));
      } catch (IOException e) {
        throw new HiveException(e);
      }
    }
