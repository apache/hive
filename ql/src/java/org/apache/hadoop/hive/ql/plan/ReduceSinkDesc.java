...
public class ReduceSinkDesc extends AbstractOperatorDesc {
  ...
  /**
   * The hash function that generates hash codes for partitioning. This is configured when the partition aware join
   * requires a special hash function, e.g. the bucket function of Iceberg. Otherwise, this is NULL and Hive uses
   * the standard hash function of Hive.
   */
  private CustomPartitionFunction customPartitionFunction;
  ...
  public void setCustomPartitionFunction(CustomPartitionFunction customPartitionFunction) {
    this.customPartitionFunction = customPartitionFunction;
  }
  ...
  public ToIntFunction<Object[]> getPartitionFunction(ObjectInspector[] objectInspectors) {
    ...
    if (customPartitionFunction != null) {
      LOG.info("Use {} for partitioning", customPartitionFunction);
      customPartitionFunction.initialize(objectInspectors);
      Preconditions.checkState(customPartitionFunction.getObjectInspector().getCategory() == Category.PRIMITIVE);
      Preconditions.checkState(((PrimitiveObjectInspector) customPartitionFunction.getObjectInspector()).getPrimitiveCategory()
          == PrimitiveCategory.INT);
      // Currently, we support the only integer type here
      final IntObjectInspector intObjectInspector = (IntObjectInspector) customPartitionFunction.getObjectInspector();
      return (fields) -> {
        final Object partitionValue = customPartitionFunction.getPartitionValue(fields);
        return intObjectInspector.get(partitionValue);
      };
