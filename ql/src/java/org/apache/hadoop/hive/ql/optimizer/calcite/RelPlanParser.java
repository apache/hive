...
public class RelPlanParser {
  ...
  private class HiveRelInput implements RelInput {
    ...
    record DistributionKey(RelDistribution.Type type, List<Integer> keys) {}
    final Map<DistributionKey, RelDistribution> distributionCache = new HashMap<>();
    ...
    public <E extends Enum<E>> E getEnum(String tag, Class<E> enumClass) {
      return Util.enumVal(enumClass, requireNonNull(getString(tag)).toUpperCase(Locale.ROOT));
    }
    ...
    public RelDataType getRowType(String expressionsTag, String fieldsTag) {
      ...
      return cluster.getTypeFactory().createStructType(new AbstractList<>() {
        @Override
    ...
    public RelDistribution getDistribution() {
      // we cannot call relJson.toDistribution here as it calls RelDistributions.of(type, list);
      // which throws an AssertionError when the keys don't have a natural ordering. This fails for
      // tests like mapreduce5.q
      
      // We need to cache the distribution objects as they are called multiple times in the 
      // constructor of HiveSortExchange. The Calcite's RelDistributionTraitDef.INSTANCE.canonize() 
      // method doesn't seem to be caching them and this results in an AssertionError in
      // RelTraitSet.containsIfApplicable later on.
      // This can also be fixed by upgrading to Java 22+ which allows statements before this() or 
      // super() in a constructor. We can then call this method once and reuse the result in other
      // places in the constructor.
      Map<String, Object> distribution = (Map<String, Object>) get("distribution");
      RelDistribution.Type type = Util.enumVal(RelDistribution.Type.class, 
          requireNonNull((String) requireNonNull(distribution).get("type")));
      List<Integer> keys = (List<Integer>) distribution.get("keys");
      DistributionKey distKey = new DistributionKey(type, keys);

      return distributionCache.computeIfAbsent(distKey, k -> new HiveRelDistribution(type, keys));
    }
