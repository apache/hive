...
public class ExprWalkerInfo implements NodeProcessorCtx {
  ...
  /**
   *  this map contains a expr infos. Each key is a node in the expression tree and the 
  ...
  public ExprWalkerInfo(Operator<? extends Serializable> op, final RowResolver toRR) {
    this.op = op;
    this.toRR   = toRR;
  ...
  /**
   * @return converted expression for give node. If there is none then returns null.
   */
  ...
  /**
   * Returns the list of pushdown expressions for each alias that appear in the current operator's
   ...
   * @param ewi ExpressionWalkerInfo
   */
