...
@Description(name = "bitmap_max", value = "_FUNC_(x) - Calculate and return the max values of a bitmap.\n" ,
extended = "Example:\n > select _FUNC_(bitmap_from_string('-2147483648,-1,1,2,3')) from src limit 1;\n" +
    "3")
public class UDFBitmapMax extends GenericUDF {
  public final static String FUNC_NAME = "bitmap_max";
  private static final Logger LOG = LoggerFactory.getLogger(UDFBitmapMax.class.getName());
  ...
  @Override public Object evaluate(DeferredObject[] arguments) throws HiveException {
    try {
      ...
      if (bitmap.isEmpty()){
        ...
        return bitmap.first()<0?bitmap.last():bitmap.select(bitmap.rank(Integer.MAX_VALUE) - 1);
      }
    } catch (IOException e) {
