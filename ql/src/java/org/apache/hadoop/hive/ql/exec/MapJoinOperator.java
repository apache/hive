...
public class MapJoinOperator extends AbstractMapJoinOperator<MapJoinDesc> implements Serializable {
  ...
  private void correctSmallTableValueObjectInspectors() {
    ...
    for (Byte alias : order) {
      corrected |= correctAliasValueObjectInspectors(alias);
    }
    ...
    outputObjInspector = ObjectInspectorFactory.getStandardStructObjectInspector(
        conf.getOutputColumnNames(), flattenValueObjectInspectors());
  }

  /**
   * Corrects the value field object inspectors for a single small table alias in-place in
   * joinValuesStandardObjectInspectors, using the position-based MapJoinDesc#getValueIndex(alias).
   * Returns whether any field was actually corrected.
   */
  private boolean correctAliasValueObjectInspectors(Byte alias) {
    int[] valueIndex = conf.getValueIndex(alias);
    ...
    if (valueIndex == null || aliasOIs == null
        ...
        || !(inputObjInspectors[alias] instanceof StructObjectInspector aliasStructOI)) {
      return false;
    }
  ...
  private List<ObjectInspector> flattenValueObjectInspectors() {
