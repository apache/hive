...
public class JDBCFilterJoinRule extends FilterJoinRule.FilterIntoJoinRule {

  // @formatter:off
  private static final FilterJoinRule.FilterIntoJoinRule.FilterIntoJoinRuleConfig DEFAULT =
      (FilterIntoJoinRuleConfig) FilterIntoJoinRuleConfig.DEFAULT
          .withOperandSupplier(
              b0 -> b0.operand(HiveFilter.class).oneInput(
                  b1 -> b1.operand(HiveJoin.class).inputs(
                      b2 -> b2.operand(HiveJdbcConverter.class).anyInputs(),
                      b3 -> b3.operand(HiveJdbcConverter.class).anyInputs())))
          .withRelBuilderFactory(HiveRelFactories.HIVE_BUILDER)
          .withDescription("JDBCFilterJoinRule");
  // @formatter:on
