 ...
 *
 * This differs from the FilterTableFunctionTransposeRule for a number of reasons:
 *   - We check if the filter condition is deterministic under Hive rules. We cannot
 *     push filters that aren't deterministic.
 *   - We check if the filter only references columns that are not part of the udtf.
 *     If the input ref in the filter references a column that is generated by the
