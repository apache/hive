...
public class IndexListInputFormat extends FileInputFormat<NullWritable, Text> {

  private Configuration conf;
  private Path indexBase;
  ...
  public RecordReader<NullWritable, Text> createRecordReader(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
    ...
    return new IndexListReader(indexSplit, context);
  }
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    List<FileStatus> statuses = listStatus(job);
    ...
    for (FileStatus status : statuses) {
      ...
      IndexSplit split = new IndexSplit(path);
      splits.add(split);
    }
