...
public class ScheduledQueryAnalyzer extends BaseSemanticAnalyzer {
  ...
  private Timestamp parseTimeStamp(Tree offsetNode) {
    ...
    String unescapeSQLString = unescapeSQLString(offsetNode.getText());
    return parseTimeStamp(unescapeSQLString);
  }

  private static Timestamp parseTimeStamp(String unescapeSQLString) {
    List<String> s = new ArrayList<>();
    s.add("YYYY-MM-DD HH:mm:ss-ZZZZ");
    s.add("EEE, dd MMM yyyy HH:mm:ss 'GMT'");
    s.add("HH:mm:ss");
    s.add("H:mm:ss");
    s.add("HH:mm");

    TimestampParser p = new TimestampParser(s);
    return p.parseTimestamp(unescapeSQLString);
  }
