...
abstract class AbstractAddPartitionAnalyzer extends AbstractAlterTableAnalyzer {
  ...
  protected void analyzeCommand(TableName tableName, Map<String, String> partitionSpec, ASTNode command)
    ...
    AlterTableAddPartitionDesc desc =
        new AlterTableAddPartitionDesc(table.getDbName(), table.getTableName(), ifNotExists);
    createPartitions(desc, command, table, ifNotExists);
    if (desc.getPartitionCount() == 0) { // nothing to do
      return;
    }
  ...
  private void createPartitions(AlterTableAddPartitionDesc desc, ASTNode command, Table table, boolean ifNotExists)
      throws SemanticException {
    ...
    for (int num = ifNotExists ? 1 : 0; num < command.getChildCount(); num++) {
      ...
      case HiveParser.TOK_PARTSPEC:
        if (currentPart != null) {
          createPartitionDesc(desc, table, currentLocation, currentPart);
  ...
  private void createPartitionDesc(AlterTableAddPartitionDesc desc, Table table, String location,
      Map<String, String> partitionSpec) {
    Map<String, String> params = null;
    if (conf.isAutogatherStatsEnabled() && location == null) {
