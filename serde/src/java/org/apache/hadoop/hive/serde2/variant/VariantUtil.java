...
public class VariantUtil {
  ...
  public static final int SIZE_LIMIT =
      isTesting() ? U24_MAX + 1 : 128 * 1024 * 1024;
  ...
  static RuntimeException unknownPrimitiveTypeInVariant(int id) {
    return new RuntimeException("UNKNOWN_PRIMITIVE_TYPE_IN_VARIANT" +
        ImmutableMap.of("id", Integer.toString(id)));
  }
  ...
  public static Type getType(byte[] value, int pos) {
    ...
    return switch (basicType) {
      ...
      default -> switch (typeInfo) {
        ...
        case UUID -> Type.UUID;
        default -> throw unknownPrimitiveTypeInVariant(typeInfo);
      };
    };
  }
  ...
  public static long getLong(byte[] value, int pos) {
    ...
    return switch (typeInfo) {
      case INT1 -> readLong(value, pos + 1, 1);
      case INT2 -> readLong(value, pos + 1, 2);
      case INT4, DATE -> readLong(value, pos + 1, 4);
      case INT8, TIMESTAMP, TIMESTAMP_NTZ -> readLong(value, pos + 1, 8);
  ...
  /**
   * Parse metadata to extract dictionary strings.
   * Returns a list of dictionary strings from the variant metadata.
   */
  public static List<String> parseMetadata(byte[] metadata) {
    ...
    try {
      ...
      for (int i = 0; i < dictSize; i++) {
        dictionary.add(getMetadataKey(metadata, i));
      }
    } catch (Exception e) {
  ...
  private static boolean isTesting() {
    return System.getenv("HIVE_IN_TEST") != null || System.getProperty("hive.in.test") != null;
  }
}