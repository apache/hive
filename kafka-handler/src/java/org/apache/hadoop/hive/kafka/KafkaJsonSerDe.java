...
public class KafkaJsonSerDe extends AbstractSerDe
{
  ...
  public static Function<TypeInfo, ObjectInspector> typeInfoToObjectInspector = typeInfo -> PrimitiveObjectInspectorFactory
  ...
  public void initialize(
      @Nullable Configuration conf, Properties tbl
  )
  {
  ...
  {
    switch (TypeInfoFactory.getPrimitiveTypeInfo(typeInfo.getTypeName())
      ...
      case TIMESTAMPLOCALTZ:
        ...
        return new TimestampLocalTZWritable(new TimestampTZ(ZonedDateTime
                                                                .ofInstant(
                                                                    Instant.ofEpochMilli(numberOfMillis),
      ...
      case CHAR:
        return (new HiveCharWritable(
            new HiveChar(value.textValue(), ((CharTypeInfo) typeInfo).getLength())));
      case VARCHAR:
        return (new HiveVarcharWritable(
            new HiveVarchar(value.textValue(), ((CharTypeInfo) typeInfo).getLength())));
      case STRING:
  ...
  private Map<String, JsonNode> parseAsJson(byte[] value) throws IOException
  {
    ...
    document.fields()
            .forEachRemaining(field -> documentMap.put(field.getKey().toLowerCase(), field.getValue()));
    return documentMap;
  }
