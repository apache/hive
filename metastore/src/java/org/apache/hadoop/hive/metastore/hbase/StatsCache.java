...
class StatsCache {
  ...
  // How often to run the invalidator, in milliseconds.
  private static final long DEFAULT_RUN_INVALIDATOR_EVERY = 5000;
  // Maximum amount of time in milliseconds an entry can live in the HBase cache.  This is to avoid
  // cruft building up over the centuries.
  private static final long DEFAULT_MAX_TIME_IN_CACHE = 1000 * 84600 * 7;
  ...
  private long runInvalidatorEvery = DEFAULT_RUN_INVALIDATOR_EVERY;
  private long maxTimeInCache = DEFAULT_MAX_TIME_IN_CACHE;
  ...
  private StatsCache(Configuration conf) {
    ...
    cache = CacheBuilder.newBuilder()
        .maximumSize(
            HiveConf.getIntVar(conf, HiveConf.ConfVars.METASTORE_HBASE_STATS_CACHE_OBJECTS))
        .expireAfterWrite(HiveConf.getTimeVar(conf,
            HiveConf.ConfVars.METASTORE_HBASE_STATS_CACHE_TTL, TimeUnit.SECONDS), TimeUnit.SECONDS)
        .build(new CacheLoader<StatsCacheKey, AggrStats>() {
          ...
          public AggrStats load(StatsCacheKey key) throws Exception {
            ...
            AggrStats aggrStats = hrw.getAggregatedStats(key.md5);
            if (aggrStats == null) {
              misses.incr();
              ...
              List<ColumnStatistics> css = hrw.getPartitionStatistics(key.dbName, key.tableName,
                  key.partNames, HBaseStore.partNameListToValList(key.partNames),
  ...
  void put(StatsCacheKey key, AggrStats aggrStats) throws IOException {
    HBaseReadWrite.getInstance().putAggregatedStats(key.md5, key.dbName, key.tableName,
        key.partNames,
  ...
  AggrStats get(String dbName, String tableName, List<String> partNames, String colName)
      throws IOException {
    totalGets.incr();
    StatsCacheKey key = new StatsCacheKey(dbName, tableName, partNames, colName, md);
    try {
