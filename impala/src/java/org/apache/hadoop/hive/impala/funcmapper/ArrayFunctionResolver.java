...
/**
 * Function resolving for the Impala "array"
 */
public class ArrayFunctionResolver extends ImpalaFunctionResolverImpl {

  public ArrayFunctionResolver(FunctionHelper helper, SqlOperator operator,
      List<RexNode> inputNodes) {
    super(helper, operator, inputNodes);
  }
  ...
  public List<RexNode> getConvertedInputs(ImpalaFunctionSignature function) throws HiveException {
    return castToCompatibleType(inputNodes);
  }

  @Override
  public RelDataType getRetType(ImpalaFunctionSignature function, List<RexNode> inputs) {
    return rexBuilder.getTypeFactory().createArrayType(inputs.get(0).getType(), -1);
  }
