...
public class QOutProcessor {
  ...
  public static final String MASK_SIZE_PATTERN = "$1#Masked#$2";
    ...
    return new ArrayList<Pair<Pattern, String>>() {
      {
        ...
        add(toPatternPair("(.*totalSize\\s*=*\\s*)\\d+\\s*(.*)", MASK_SIZE_PATTERN));
        add(toPatternPair("(.*File raw data size:*\\s*)\\d+\\s*bytes(.*)", MASK_SIZE_PATTERN));
