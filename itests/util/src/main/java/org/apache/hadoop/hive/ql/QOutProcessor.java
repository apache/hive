...
public class QOutProcessor {
  ...
  public static final String MASK_1 = "$1#Masked#";
  public static final String MASK_2 = "$1#Masked#$2";
    ...
    return new ArrayList<Pair<Pattern, String>>() {
      {
        ...
        add(toPatternPair("(.*totalSize\\s*=*\\s*)\\d+\\s*(.*)", MASK_2));
        add(toPatternPair("(.*File raw data size:*\\s*)\\d+\\s*bytes(.*)", MASK_2));
        add(toPatternPair("(totalFileSize:\\s*)\\d+", MASK_1));
        add(toPatternPair("(maxFileSize:\\s*)\\d+", MASK_1));
        add(toPatternPair("(minFileSize:\\s*)\\d+", MASK_1));
      }
    };
  }
