...
public class QTestSysDbHandler implements QTestOptionHandler {
  ...
  public void beforeTest(QTestUtil qt) throws Exception {
    if (enabled) {
      String sv = MetastoreVersionInfo.getShortVersion();
      String stsdbPath = HiveTestEnvSetup.HIVE_ROOT + "/metastore/scripts/upgrade/hive/hive-schema-" + sv + ".hive.sql";
      if (!new File(stsdbPath).exists()) {
        throw new RuntimeException("metastore schema file doesnt exists:" + stsdbPath);
      }
