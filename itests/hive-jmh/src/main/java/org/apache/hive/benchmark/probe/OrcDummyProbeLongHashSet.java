...
public class OrcDummyProbeLongHashSet extends OrcProbeHashTable {
  ...
  public OrcDummyProbeLongHashSet(VectorMapJoinHashTable vTable, VectorMapJoinInfo vInfo) {
    super(vTable, vInfo);
  ...
  public void filterColumnVector(ColumnVector cv, MutableFilterContext cntx, int batchSize) {
    ...
    try {
      if (probeCol.isRepeating) {
        newSize = probeLongHashSet.contains(probeCol.vector[0], hashSetResult) == JoinUtil.JoinResult.MATCH ? batchSize : 0;
      } else {
        for (int row = 0; row < batchSize; ++row) {
          newSize += probeLongHashSet.contains(probeCol.vector[row], hashSetResult) == JoinUtil.JoinResult.MATCH ? 1 : 0;
        }
        selectedInUse = true;
      }
