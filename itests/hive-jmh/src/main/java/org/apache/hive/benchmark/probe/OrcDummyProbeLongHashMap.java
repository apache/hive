...
public class OrcDummyProbeLongHashMap extends OrcProbeHashTable {
  ...
  public OrcDummyProbeLongHashMap(VectorMapJoinHashTable vTable, VectorMapJoinInfo vInfo) {
    super(vTable, vInfo);
  ...
  public void filterColumnVector(ColumnVector cv, MutableFilterContext cntx, int batchSize) {
    ...
    try {
      if (probeCol.isRepeating) {
        newSize = probeLongHashMap.lookup(probeCol.vector[0], hashMapResult) == JoinUtil.JoinResult.MATCH ? batchSize : 0;
      } else {
        for (int row = 0; row < batchSize; ++row) {
          newSize += probeLongHashMap.lookup(probeCol.vector[row], hashMapResult) == JoinUtil.JoinResult.MATCH ? 1 : 0;
        }
        selectedInUse = true;
      }
