...
public class OrcDummyProbeStringHashSet extends OrcProbeHashTable {

  private VectorMapJoinBytesHashSet probeStringHashSet;
  private VectorMapJoinHashSetResult hashSetResult;

  public OrcDummyProbeStringHashSet(VectorMapJoinHashTable vTable, VectorMapJoinInfo vInfo) {
    super(vTable, vInfo);
    this.probeStringHashSet = (VectorMapJoinBytesHashSet) probeDecodeMapJoinTable;
    this.hashSetResult = probeStringHashSet.createHashSetResult();
  }
  ...
  public void filterColumnVector(ColumnVector cv, MutableFilterContext cntx, int batchSize) {
    ...
    try {
      if (probeCol.isRepeating) {
        newSize = probeStringHashSet.
            contains(probeCol.vector[0], probeCol.start[0], probeCol.length[0], hashSetResult)
            == JoinUtil.JoinResult.MATCH ? batchSize : 0;
      } else {
        for (int row = 0; row < batchSize; ++row) {
          newSize += probeStringHashSet.
              contains(probeCol.vector[row], probeCol.start[row], probeCol.length[row], hashSetResult)
