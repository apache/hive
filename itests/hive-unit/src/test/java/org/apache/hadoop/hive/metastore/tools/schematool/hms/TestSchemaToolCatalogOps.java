...
public class TestSchemaToolCatalogOps {
  ...
  @BeforeClass
  public static void initDb() throws IOException, HiveMetaException, ParseException {
    conf = new HiveConf();
    ...
    MetastoreConf.setVar(conf, MetastoreConf.ConfVars.CONNECT_URL_KEY, "jdbc:derby:" + testMetastoreDB + ";create=true");
    ...
    argsBase = "-dbType derby ";

    catalogLocation = Files.createTempDir();

    // Pre-install the database so all the tables are there.
    SchemaToolTestUtil.executeCommand(conf, new String[] {"-dbType", "derby", "-userName", userName,
        "-passWord ", passWord, "-initSchema"});
  }
  ...
  @Test
  public void moveDbToNonExistentCatalog() throws TException {
  ...
  @Test
  public void moveTableWithExistingTableOfSameNameAlreadyInTargetDatabase() throws TException {
  ...
  @Test
  public void moveTableToNonExistentDb() throws TException {
  ...
  private void execute(String taskArgs) throws HiveMetaException {
    try {
      StringTokenizer tokenizer = new StringTokenizer(argsBase + taskArgs, ' ', '\"');
      SchemaToolTestUtil.executeCommand(conf, tokenizer.getTokenArray());
    } catch (ParseException e) {
      Assert.fail("Unable to parse the task arguments");
    }
  }
