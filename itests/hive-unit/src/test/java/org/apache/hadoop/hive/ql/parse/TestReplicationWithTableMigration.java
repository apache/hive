...
public class TestReplicationWithTableMigration {
  ...
  public void testMigrarionLocationOwnership() throws Throwable {
    ...
    assertTrue(isExternal(replica.getTable(replicatedDbName, "tbl")));
    assertTrue(isExternal(replica.getTable(replicatedDbName, "tbl_part")));
    assertFalse(isExternal(primary.getTable(primaryDbName, "tbl")));
    assertFalse(isExternal(primary.getTable(primaryDbName, "tbl_part")));

    Path newLoc = new Path(location.getParent(), "tbl_inc");
    assertTrue(fs.mkdirs(newLoc));
    fs.setOwner(newLoc, "junk", "junk");

    Path newLocPart = new Path(location.getParent(), "tbl_inc_part_3");
    ...
    assertTrue(isExternal(replica.getTable(replicatedDbName, "tbl_inc")));
    assertFalse(isExternal(primary.getTable(primaryDbName, "tbl_inc")));
    assertTrue(isExternal(replica.getTable(replicatedDbName, "tbl_inc_part")));
    assertFalse(isExternal(primary.getTable(primaryDbName, "tbl_inc_part")));
  }
}
