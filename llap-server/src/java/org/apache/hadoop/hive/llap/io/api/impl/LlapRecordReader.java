...
class LlapRecordReader
  ...
  private LlapRecordReader(MapWork mapWork, JobConf job, FileSplit split,
    ...
    this.includes = new IncludesImpl(tableIncludedCols, isAcidFormat, rbCtx, schema, job,
  ...
  public boolean next(NullWritable key, VectorizedRowBatch vrb) throws IOException {
    ...
    if (isAcidFormat) {
      ...
      if (isVectorized) {
        ...
        /**Acid meta cols are always either all included or all excluded the the width of 'cvb'
        changes accordingly so 'acidColCount' and 'ixInVrb' need to be adjusted.
         See {@link IncludesImpl} comments.*/
        ...
        VectorizedRowBatch inputVrb = new VectorizedRowBatch(
            acidColCount + 1 + vrb.getDataColumnCount());//so +1 is the OrcRecordUpdater.ROW?
   ...
   *
   *  There is some split brain issue between {@link SchemaEvolution} used in non-LLAP path and
   *  this class.  The file schema for acid tables looks like this and
   *  <op, owid, writerId, rowid, cwid, <f1, ... fn>> and {@link SchemaEvolution#getFileIncluded()}
   *  respects that.  So if fn=2, the type IDs are 0..8 and the fileIncluded[] has 9 bits that
   *  indicate what is read.  So in particular, {@link org.apache.hadoop.hive.ql.io.orc.RecordReader}
   *  produces ColumnVectorS are NULL in every row for each fileIncluded[9]==false.  The fields
   *  corresponding to structs are always included if any child of the struct has to be included.
   *
   *  LLAP only produces ColumnVectorS if they are needed so the width of ColumnVectorBatch varies
   *  depending on what was projected.
   *
      ...
      if (isAcidScan) {
        ...
        for (int i = 0; i < rootCol; ++i) {
          ...
          if(!includeAcidColumns) {
            /**if not including acid columns, we still want to number the physical columns as if
            * acid columns are included becase {@link #generateFileIncludes(TypeDescription)} takes
             * the file schema as input (eg <op, owid, writerId, rowid, cwid, <f1, ... fn>>)*/
            continue;
          }
          filePhysicalColumnIds.add(i);
        }
        for (int tableColumnId : readerLogicalColumnIds) {
          filePhysicalColumnIds.add(rootCol + tableColumnId);//but make sure to generate correct ids in type tree in-order walk order
        }
