...
public class OrcProbeStringHashMap extends OrcProbeHashTable {
  ...
  public void filterColumnVector(ColumnVector cv, MutableFilterContext cntx, int batchSize) {
    ...
    try {
      ...
      LlapIoImpl.LOG.debug("ProbeDecode StrMap Matched: {} selectedInUse {} batchSize {}", newSize, selectedInUse, batchSize);
    } catch (IOException e) {
      LlapIoImpl.LOG.error("ProbeDecode StrMap Filter failed: {}", e);
      e.printStackTrace();
    }
  }
}
