...
public class OrcProbeStringHashSet extends OrcProbeHashTable {
  ...
  public OrcProbeStringHashSet(VectorMapJoinHashTable vTable, VectorMapJoinInfo vInfo) {
    super(vTable, vInfo);
    ...
    try {
      if (probeCol.isRepeating) {
        ...
        if (isNull) {
          ...
          if (probeStringHashSet.contains(vector[0], start[0], length[0], hashSetResult) == JoinUtil.JoinResult.MATCH) {
