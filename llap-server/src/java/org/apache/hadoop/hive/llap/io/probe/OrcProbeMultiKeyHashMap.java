    ...
    try {
      if (probeCol.isRepeating) {
        ...
        if (!probeCol.noNulls && probeCol.isNull[0]) {
          ...
          if (probeHashMultiMap.lookup(currKeyOutput.getData(), 0, currKeyOutput.getLength(),
      ...
      LlapIoImpl.LOG.debug("ProbeDecode MultiKeyMap Matched: {} selectedInUse {} batchSize {}", newSize, selectedInUse, batchSize);
    } catch (IOException e) {
      LlapIoImpl.LOG.error("ProbeDecode MultiKeyMap Filter failed: {}", e);
      e.printStackTrace();
    }
  }
}
