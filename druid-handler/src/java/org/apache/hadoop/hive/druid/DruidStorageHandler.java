...
public class DruidStorageHandler extends DefaultHiveMetaHook implements HiveStorageHandler {
  ...
  public void publishSegments(Table table, boolean overwrite) throws MetaException {
    ...
    LOG.info("Committing hive table {} druid data source {} to the druid metadata store",
            table.getDbName(), dataSourceName
    );
    try {
    ...
    try {
      LOG.info("Found {} segments under path {}", segmentList.size(), getStagingWorkingDir());
      ...
      console.logInfo(
              String.format("Moving [%s] druid segments from [%s] to [%s] then committing metadata in progress",
                      segmentList.size(),
      ...
      console.logInfo("waiting for segments loading by checking load status from coordinator {}",
              coordinatorAddress
      );
