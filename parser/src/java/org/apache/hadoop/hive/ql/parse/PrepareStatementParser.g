...
@after { gParent.popMsg(state); }
    : LPAREN executeParam (COMMA executeParam)* RPAREN
    -> ^(TOK_EXECUTE_PARAM_LIST executeParam+)
    ;

executeParam
@init { gParent.pushMsg("execute param", state); }
@after { gParent.popMsg(state); }
    : Number COMMA constant
    -> ^(TOK_EXECUTE_PARAM Number constant)
    ;
