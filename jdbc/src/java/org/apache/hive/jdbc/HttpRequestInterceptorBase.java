...
public abstract class HttpRequestInterceptorBase implements HttpRequestInterceptor {
  ...
  public void process(HttpRequest httpRequest, EntityDetails entity, HttpContext httpContext)
    ...
    try {
      generateKerberosTicket(httpRequest, httpContext);

      if (isCookieEnabled) {
        httpContext.setAttribute(Utils.HIVE_SERVER2_RETRY_KEY, Utils.HIVE_SERVER2_CONST_FALSE);
      }

      handleRequesTracking(httpContext);

      insertAdditionalHttpHeaders(httpRequest);

      addCustomCookiesIfPassedToTheJdbcDriver(httpRequest);
    } catch (Exception e) {
      throw new HttpException(e.getMessage(), e);
    }
  }

  private void addCustomCookiesIfPassedToTheJdbcDriver(HttpRequest httpRequest) {
  ...
  private void insertAdditionalHttpHeaders(HttpRequest httpRequest) {
  ...
  private void handleRequesTracking(HttpContext httpContext) {
  ...
  private void generateKerberosTicket(HttpRequest httpRequest, HttpContext httpContext) throws Exception {
