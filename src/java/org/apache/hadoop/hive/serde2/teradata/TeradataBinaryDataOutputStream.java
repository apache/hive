...
public class TeradataBinaryDataOutputStream extends ByteArrayOutputStream {
   ...
   * @throws IOException the io exception
   */
  public void writeTimestamp(TimestampWritable timestamp, int byteNum) throws IOException {
    if (timestamp == null) {
      ...
      if (sTimeStamp.length() >= byteNum) {
        ...
        if (sTimeStamp.length() == TIMESTAMP_NO_NANOS_BYTE_NUM) {
          pad = "." + StringUtils.repeat('0', byteNum - sTimeStamp.length() - 1);
        } else {
  ...
  public void writeDate(DateWritable date) throws IOException {
    if (date == null) {
      ...
      EndianUtils.writeSwappedInteger(this, Integer.parseInt(dateFormat.format(date.get())) - 19000000);
    }
  }
  ...
  public void writeDecimal(HiveDecimalWritable writable, int byteNum, int scale) throws IOException {
    ...
    if (hiveScale < scale) {
      BigInteger multiplicand = new BigInteger("1" + StringUtils.repeat('0', scale - hiveScale));
