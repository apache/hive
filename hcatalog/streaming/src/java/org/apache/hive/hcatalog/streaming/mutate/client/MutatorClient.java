    ...
    for (AcidTable table : tables) {
      try {
        table.setWriteId(metaStoreClient.allocateTableWriteId(txnId,
                new TableName(Warehouse.DEFAULT_CATALOG_NAME, table.getDatabaseName(),
                    table.getTableName())));
      } catch (TException ex) {
        try {
          ...
          LOG.warn("Allocation of write id failed for table {} and rollback transaction {} failed due to {}",
              TableName.getDbTable(table.getDatabaseName(), table.getTableName()), txnId, e.getMessage());
        }
        throw new TransactionException("Unable to allocate table write ID for table "
                + TableName.getDbTable(table.getDatabaseName(), table.getTableName())
                + " under txn " + txnId, ex);
      }
    }
