...
public class ZooKeeperStorage implements TempletonStorage {
  ...
  public static CuratorFramework zkOpen(Configuration conf) throws IOException {
    ...
    int sessionTimeoutMs = conf.getInt(ZK_SESSION_TIMEOUT, CuratorFrameworkFactory.builder().getSessionTimeoutMs());
    boolean sslEnabled = conf.getBoolean(ZK_SSL_ENABLE, false);
    String keyStoreLocation = conf.get(ZK_KEYSTORE_LOCATION, "");
    String keyStorePassword = conf.get(ZK_KEYSTORE_PASSWORD, "");
    String trustStoreLocation = conf.get(ZK_TRUSTSTORE_LOCATION, "");
    String trustStorePassword = conf.get(ZK_TRUSTSTORE_PASSWORD, "");
