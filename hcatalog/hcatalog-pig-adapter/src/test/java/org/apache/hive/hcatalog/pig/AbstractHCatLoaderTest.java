...
public abstract class AbstractHCatLoaderTest extends HCatBaseTest {
  ...
  public void testSchemaLoadComplex() throws IOException {
    ...
    assertEquals("name", Kfields.get(0).alias);

    assertEquals(DataType.INTEGER, Kfields.get(1).type);
    assertEquals("studentid", Kfields.get(1).alias);

    assertEquals(DataType.TUPLE, Kfields.get(2).type);
    assertEquals("contact", Kfields.get(2).alias);
    {
    ...
    assertEquals("currently_registered_courses", Kfields.get(3).alias);
    {
      ...
      // assertEquals("course",Kfields.get(3).schema.getFields().get(0).schema.getFields().get(0).alias);
    ...
    assertEquals("current_grades", Kfields.get(4).alias);
    assertEquals(DataType.BAG, Kfields.get(5).type);
    assertEquals("phnos", Kfields.get(5).alias);
    {
      ...
      assertEquals("phno", Kfields.get(5).schema.getFields().get(0).schema.getFields().get(0).alias);
      assertEquals(DataType.CHARARRAY, Kfields.get(5).schema.getFields().get(0).schema.getFields().get(1).type);
      assertEquals("type", Kfields.get(5).schema.getFields().get(0).schema.getFields().get(1).alias);
    }
