...
public class TestHCatMultiOutputFormat {
  ...
  public void testOutputFormat() throws Throwable {
    ...
    for (int i = 0; i < tableNames.length; i++) {
      ...
      final Path parentOfPartitionFile = partitionFile;
      final Path grandParentOfPartitionFile = parentOfPartitionFile.getParent();
      ...
      Assert.assertEquals(
          "File permissions of table " + tableNames[i] + " is not correct [" + parentOfPartitionFile + "]",
          new FsPermission(tablePerms[i]), fs.getFileStatus(parentOfPartitionFile).getPermission());
      Assert.assertEquals(
